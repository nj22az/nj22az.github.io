...
function draw(time){
 ...
 if(lastScene!==s.id){lastScene=s.id;$('#scene-title').textContent=s.title;$('#slide-reference').textContent='PowerPoint bild '+s.slides.join(', ');canvas.setAttribute('aria-label',`${s.title}. ${s.key}`);document.querySelectorAll('#chapters button').forEach(b=>{if(Number(b.dataset.chapter)===s.chapter)b.setAttribute('aria-current','true');else b.removeAttribute('aria-current')});history.replaceState(null,'',`#${s.id}`);}
...
function pause(){playing=false;voice.pause();music.pause();cancelAnimationFrame(raf);$('#play').textContent='Spela';$('#play').setAttribute('aria-pressed','false')}
async function play(){
 if(!ready)return;if(voice.currentTime>=timeline.duration-.1)seek(0);
 try{await Promise.all([voice.play(),music.play()]);playing=true;$('#play').textContent='Pausa';$('#play').setAttribute('aria-pressed','true');$('#status').textContent='';raf=requestAnimationFrame(frame)}
 catch(e){pause();$('#status').textContent='Ljudet kunde inte starta. Tryck på Spela igen eller kontrollera anslutningen.';}
}
...
$('#fullscreen').addEventListener('click',async()=>{try{if(document.fullscreenElement)await document.exitFullscreen();else await $('#viewer').requestFullscreen()}catch{$('#status').textContent='Helskärm stöds inte här. Använd webbläsarens helskärmsläge.'}});
...
for(const a of [voice,music])a.addEventListener('error',()=>{pause();$('#status').textContent='En ljudfil kunde inte läsas. Ladda om sidan för att försöka igen.'});
...
const transcript=$('#transcript');timeline.scenes.forEach(s=>{const section=document.createElement('section'),h=document.createElement('h3'),p=document.createElement('p'),b=document.createElement('button');h.textContent=s.title;p.textContent=s.say;b.textContent=`Spela från ${fmt(s.start)}`;b.type='button';b.addEventListener('click',()=>{seek(s.start);play();$('#viewer').scrollIntoView({block:'start'})});section.append(h,p,b);transcript.append(section)});
...
$('#play').disabled=false;$('#status').textContent='Redo. Tryck på Spela för film och ljud.';
