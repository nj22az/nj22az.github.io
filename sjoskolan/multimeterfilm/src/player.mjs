...
function draw(time){
 ...
 if(lastScene!==s.id){lastScene=s.id;$('#scene-title').textContent=s.title;$('#slide-reference').textContent='PowerPoint slide '+s.slides.join(', ');canvas.setAttribute('aria-label',`${s.title}. ${s.key}`);document.querySelectorAll('#chapters button').forEach(b=>{if(Number(b.dataset.chapter)===s.chapter)b.setAttribute('aria-current','true');else b.removeAttribute('aria-current')});history.replaceState(null,'',`#${s.id}`);}
