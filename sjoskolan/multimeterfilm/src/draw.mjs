/** Multimeter Aboard — original marker-comic artwork, entirely Canvas paths.
...
function sectionBase(c,s,t){
 ...
 txt(c,'SJÖSKOLAN',82,61,24,C.blue,800);txt(c,'MULTIMETER ABOARD',1838,61,22,C.grey,600,'right');
 ...
 txt(c,`PowerPoint slide ${s.slides.length>1?s.slides[0]+'–'+s.slides.at(-1):s.slides[0]}`,1836,925,20,C.grey,400,'right');
}
...
function numberline(c,{min,max,a,b,center,x=180,y=570,width=1410,color=C.teal,t=5,label=''}){
 const pos=v=>x+(v-min)/(max-min)*width;line(c,[[x,y],[x+width,y]],C.ink,4);for(let j=0;j<=8;j++){const v=min+(max-min)*j/8,px=pos(v);line(c,[[px,y-10],[px,y+10]],C.ink,3);txt(c,v.toFixed(2),px,y+50,24,C.grey,400,'center')}
...
export function drawScene(c,s,t,env){
 ...
 if(v==='intro'||v==='outro'){
  ...
  wrap(c,end?'A measurement you can explain.':'Ask first. Then measure.',1305,707,480,29,C.ink,700);
  if(end){box(c,840,362,352,245,7,C.white,C.ink,4);txt(c,'MEASUREMENT LOG',862,405,24,C.blue,700);['P1–P2 · V DC','11.80 V ±0.08 V','Limits 11.40–12.60 V'].forEach((a,i)=>txt(c,a,862,457+i*45,24));}
  else {txt(c,'Saga',332,821,27,C.blue,700,'center');txt(c,'The Gull',1128,821,27,C.blue,700,'center');txt(c,'12 V',655,733,39,C.blue,700,'center')}
 } else if(v==='quantity'){
  ...
  if(q==='I'){stream(c,pts,t);txt(c,'Charge per second',835,527,43,C.teal,700);arrow(c,1140,315,1290,315)}
  ...
  if(v==='rig'){sailor(c,1510,783,1.12,t,true);gull(c,1785,784,.8,t);txt(c,'SELV · protective separation',255,767,33,C.blue,700)}
  else txt(c,s.powered?'SUPPLY ON AFTER CHECKING':'SUPPLY OFF WHEN RECONNECTING',275,780,27,s.powered?C.teal:C.grey,700);
 }else if(v==='meter'||v==='restore'||v==='wrong'){
  let mode=v==='meter'?(t<6?'dc':t<9?'ac':t<12?'ohm':'dc'):'dc',jack=v==='wrong'?'a':v==='restore'&&t<(s.cues?.restore??5)?'a':'v';
  meter(c,355,252,1.0,t,{mode,reading:v==='meter'?(mode==='ohm'?'120.0':'12.00'):'—',unit:mode==='ohm'?'Ω':'V',jack,highlight:v==='meter'?null:'jack'});
  const labels=v==='meter'?['Display: value + unit + mode','Selector: measurement function','COM: black lead','VΩ or A/mA: red lead']:v==='restore'?['1  Disconnect the supply','2  Restore the current path','3  Move red back to VΩ']:['The selector is on V DC.','Red is in A. What must change?','The current input can short-circuit the source.'];
  labels.forEach((l,i)=>wrap(c,l,840,335+i*112,870,37,i===2&&v==='wrong'?C.red:C.ink,600));
  if(v==='wrong'){gull(c,1100,798,1.2,t);txt(c,t<(s.cues?.answer??7)?'PAUSE AND THINK':'MOVE RED TO VΩ',1340,750,25,C.red,700)}
  else sailor(c,1665,793,.86,t,true);
 }else if(v==='check'){
  ['What will you measure, and where?','Are the meter and leads undamaged?','Correct function, sockets and range?','Have the connections been checked?'].forEach((l,i)=>{txt(c,String(i+1),177,339+i*130,49,C.blue,700);wrap(c,l,262,339+i*130,1180,40,C.ink,600);line(c,[[260,371+i*130],[1350,371+i*130]],'#d8cfbc',2,ease((t-i*.8)/1.0))});sailor(c,1627,795,1.15,t,true);
 }else if(v==='ohm'||v==='continuity'||v==='parallel'){
  const phase=v==='continuity'?(t<(s.cues?.fuse??8)?0:t<(s.cues?.open??16)?1:2):v==='parallel'&&t>(s.cues?.disconnect??14)?1:0;
  ...
  if(v==='ohm'){
   resistor(c,620,473,264,95,'R₁ = 1 kΩ');line(c,[[490,473],[595,473]]);line(c,[[909,473],[1010,473]]);node(c,490,473,'');node(c,1010,473,'');lead(c,m.black,[490,473],C.ink);lead(c,m.red,[1010,473],C.red);txt(c,'Isolated component',570,344,37,C.ink,700);
  }else if(v==='continuity'){
   txt(c,phase===0?'Probe tips together':phase===1?'Intact fuse, removed':'Open path',345,337,40,C.ink,700);
   ...
   if(phase!==2){[0,1,2].forEach(i=>{const r=24+((t*36+i*18)%62);c.save();c.globalAlpha=1-(r-24)/62;c.beginPath();c.arc(1060,460,r,-.9,.9);c.lineWidth=5;c.strokeStyle=C.teal;c.stroke();c.restore()});txt(c,'BEEP',1063,565,38,C.teal,700)}else txt(c,'OL ≠ proof that a circuit is dead',280,745,34,C.red,700);
  }else{
   ...
   txt(c,phase?'One branch disconnected: 1 000 Ω':'(1 000 · 1 000) / (1 000 + 1 000) = 500 Ω',242,774,34,C.blue,700);
  }
 }else if(v==='transient'){
  line(c,[[195,683],[1360,683]],C.ink,4);line(c,[[195,683],[195,300]],C.ink,4);txt(c,'Voltage',172,290,28,C.ink,600);txt(c,'Time',1390,694,28,C.ink,600);
  ...
  txt(c,'Brief transient',950,317,37,C.red,700);arrow(c,985,343,858,381,C.red,4);txt(c,'Operating voltage',345,758,35,C.ink,600);sailor(c,1633,787,1.05,t,true);
  ...
  ['CAT II','CAT III','CAT IV'].forEach((label,i)=>{
   ...
   if(i===2){line(c,[[x-66,413],[x-66,646],[x+70,646],[x+70,413]],C.ink,8);box(c,x-107,492,215,102,6,C.white,C.ink,4);txt(c,'SUPPLY',x,553,28,C.ink,700,'center');for(let k=0;k<3;k++){line(c,[[x-54+k*55,360],[x-54+k*55,486]],C.red,5)}}
   txt(c,['Appliances','Fixed installation','Supply origin'][i],x,730,36,C.ink,700,'center');txt(c,['Load side','Includes single phase','Low-voltage installation'][i],x,781,27,C.grey,400,'center');
  });txt(c,'Older CAT I: no direct mains connection. Check the manufacturer’s limits.',230,838,26,C.grey);
 }else if(v==='ratings'){
  meter(c,288,257,.98,t,{reading:'—'});txt(c,'CAT III 1 000 V',899,389,65,C.blue,700);txt(c,'CAT IV 600 V',899,505,65,C.blue,700);
  wrap(c,'Two rating combinations that may appear on the same instrument.',900,592,760,34);wrap(c,'Also check voltage to earth and between sockets.',900,724,760,32,C.ink,600);
 }else if(v==='impulse'){
  const xs=[160,714,1275];['RATING','BRIEF TEST IMPULSE','TEST SOURCE'].forEach((l,i)=>txt(c,l,xs[i],321,26,C.grey,700));
  ...
  txt(c,'Selected impulse-test examples; not a complete standards table.',160,793,28,C.grey);
 }else if(v==='choose'){
  txt(c,'Work-plan requirement: CAT III 600 V',200,330,46,C.ink,700);
  [['A','CAT II 1 000 V'],['B','CAT III 600 V']].forEach((r,i)=>{const x=300+i*810;txt(c,r[0],x,492,108,i?C.teal:C.grey,800);txt(c,r[1],x,585,55,i?C.teal:C.ink,700);if(t>(s.cues?.answer??7)&&i)line(c,[[x-40,625],[x+565,625]],C.teal,9,ease((t-(s.cues?.answer??7))/1.2));});
  txt(c,t<(s.cues?.answer??7)?'Pause and choose. Explain both ratings.':'B meets both category and voltage.',300,723,40,C.blue,700);gull(c,1670,798,.8,t);
 }else if(v==='resolution'){
  ['12.00','12.01'].forEach((a,i)=>{box(c,194+i*790,325,559,180,15,C.lcd,C.ink,6);txt(c,a+' V',713+i*790,458,88,C.ink,500,'right')});arrow(c,820,421,896,421,C.blue,6);txt(c,'One count = 0.01 V',940,617,61,C.blue,700,'center');
  ['Resolution: display step','Error limit: specified deviation','Tolerance: allowed range'].forEach((a,i)=>txt(c,a,200+i*565,749,i===1?28:30,C.ink,600));
 }else if(v==='accuracy'){
  ...
  txt(c,`Specification: ±(${percent} % of reading + ${digits} counts)`,150,305,35,C.grey,600);
  formula(c,[`${percent} % of ${e}.00 V = ${part} V`,`${digits} · 0.01 V = ${step} V`,`Error limit: ±(${part} + ${step}) V = ±${sum} V`],t,150,406,46,95);
  const err=Number(sum.replace(',','.'));numberline(c,{min:e-.2,max:e+.2,a:e-err,b:e+err,center:e,x:230,y:745,width:1220,color:C.mint,t:t-4,label:`${(e-err).toFixed(2)}–${(e+err).toFixed(2)} V`});gull(c,1725,805,.9,t);
 }else if(v==='tolerance'){
  txt(c,'Required: 12.0 V ±5 % = 12.0 V ±0.60 V',169,322,41,C.ink,600);
  numberline(c,{min:11.2,max:12.8,a:11.4,b:12.6,x:230,y:480,width:1410,color:C.mint,t,label:'Allowed: 11.40–12.60 V'});
  const pos=v=>230+(v-11.2)/1.6*1410;box(c,pos(11.4),621,(pos(11.6)-pos(11.4))*ease((t-2)/1.5),47,5,C.blue);
  line(c,[[pos(11.4),535],[pos(11.4),706]],C.grey,2);txt(c,'Measured: 11.50 V ±0.10 V',700,655,39,C.blue,700);
  txt(c,'The whole interval fits. Boundary values are allowed in this exercise.',230,778,35,C.ink,600);
 }else if(v==='protocol'){
  txt(c,'FICTIONAL TEACHING EXAMPLE',150,285,23,C.teal,700);
  [['Points and state','P1–P2 across H1. S1 closed, supply on.'],['Meter and range','DMM-01 · V DC · 20 V · 0.01 V/count'],['Specification','±(0.5 % of reading + 2 counts)'],['Result','11.80 V ±0.08 V → 11.72–11.88 V'],['Limits and conclusion','11.40–12.60 V. The whole interval fits.'],['Date and operator','2026-09-24, 10:15 · Student A']].forEach((r,i)=>row(c,r[0],r[1],355+i*82,1730));
 }else if(v==='divider'){
  ...
  if(has){const result=src===9?'3.0 V':s.rm===10?'4.762 V':'3.333 V';txt(c,result,1450,397,66,C.blue,700,'center');txt(c,'calculated voltage',1450,445,25,C.grey,400,'center');
  ...
  const vals=[5,4.762,3.333];const labels=['Without a meter','Rₘ = 10 MΩ','Rₘ = 1 MΩ'];
  vals.forEach((value,i)=>{const x=320+i*510;const h=310*(value/5)*ease(t/1.5);box(c,x,680-h,260,h,4,[C.ink,C.blue,C.teal][i]);txt(c,value.toFixed(3)+' V',x+130,680-h-30,44,C.ink,700,'center');txt(c,labels[i],x+130,740,31,C.ink,600,'center')});
  txt(c,'Reference: voltage before connecting the meter.',235,816,30,C.grey);
 }else if(v==='burden'){
  const pts=[[230,335],[1360,335],[1360,664],[230,664],[230,335]];line(c,pts);battery(c,230,500,true);resistor(c,480,335,236,69,'Load: 120 Ω');resistor(c,991,335,164,69,'Meter: 1 Ω');stream(c,pts,t);
  formula(c,['Without a meter: 12 / 120 = 0.100 A','With a meter: 12 / (120 + 1) ≈ 0.0992 A'],t,342,474,41,105);txt(c,'Meter voltage drop: U = I · R ≈ 0.0992 V',300,775,37,C.blue,700);gull(c,1660,779,1.1,t);
 }else if(v==='recap'){
  const labels=[['V','In parallel','COM + VΩ'],['A','In series','COM + correct A input'],['Ω / BEEP','Isolated and discharged','COM + VΩ']];
  labels.forEach((a,i)=>{const y=368+i*165;txt(c,a[0],165,y,60,C.blue,700);txt(c,a[1],545,y,39,C.ink,700);txt(c,a[2],1220,y,35,C.grey,600);line(c,[[165,y+41],[1710,y+41]],'#d8cfbc',2,ease((t-i)/1.2))});
 }else if(v==='credits'){
  const list=['Source: Multimeter och mätfel, 70 slides','Technical references: Hioki, Fluke and the instrument manual','Animation: original code, Anidoodle and Helios','Music: original composition, synthesised in Java','English narration: local Kokoro synthesis, British voice'];
