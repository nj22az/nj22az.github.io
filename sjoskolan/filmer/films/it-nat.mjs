...
function net(ctx, t, { f1 = 0, f2 = 0, sc = 0, tripped = false } = {}) {
  ...
  // isolationsvakt
  const bad = f1 > 0.5;
  line(ctx, 130, 240, 130, 330, COL.muted, 3); roundRect(ctx, 60, 330, 140, 70, 8, bad ? '#fde2e2' : '#e6f3ec', bad ? COL.red : COL.green, 3);
  text(ctx, 'isolationsvakt', 130, 350, { size: 15, weight: 700, align: 'center' }); text(ctx, bad ? 'LARM' : 'OK', 130, 378, { size: 20, weight: 700, align: 'center', color: bad ? COL.red : COL.green });
  ...
  [[C1, 0, f1], [C2, 1, f2]].forEach(([x, ph, f], k) => {
    ...
    roundRect(ctx, x + 6, 268, 28, 28, 4, '#fff', COL.ink, 3); if (off) text(ctx, 'löst', x + 42, 282, { size: 18, color: COL.red, weight: 700 });
  ...
  scenes: [
    ...
    },
    {
      ...
      say: [[0.8, 7.8, 'Sigge', 'Ombord är 440 V-nätet oftast ett IT-nät. Ingen fas är förbunden med skrovet.'],
        [8.0, 15.8, 'Sigge', 'Isolationsvakten mäter hela tiden isolationen mellan nätet och skrovet.']],
    },
    {
      dur: 22,
    ...
    },
    {
      ...
      say: [[0.8, 8.3, 'Sigge', 'Kommer ett andra fel på en annan fas blir det kortslutning mellan L1 och L2 via skrovet.'],
        ...
        [14.0, 19.8, 'Sigge', 'Endera skyddet kan lösa, eller båda. Det kan drabba viktiga förbrukare.']],
    },
    {
      ...
      say: [[0.8, 7.5, 'Sigge', 'Därför spårar vi första felet direkt enligt rutinen, till exempel med läckströmstång.'],
        [7.8, 13.5, 'Måns', 'Aldrig ignorera ett isolationslarm!'],
        [13.8, 20.3, 'Sigge', 'Sektionering görs i samråd med vakthavande. Styrmaskinen kopplas aldrig bort utan plan.'],
