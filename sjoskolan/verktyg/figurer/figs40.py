...
ax.text(10.5,1.2,"Ytorna tar ut varandra: U_{medel} = 0",ha="center",fontsize=14)
...
ax.set_xlabel("f (Hz)",color=GRAY,fontsize=12); ax.set_ylabel("X_{L} (Ω)",color=GRAY,fontsize=12)
ax.text(8,60,"L = 0,10 H\nX_{L} ökar med f",fontsize=14,color=INK,va="top"); save(F,"v40_02_s06_XL")
...
ax.set_xlabel("f (Hz)",color=GRAY,fontsize=12); ax.set_ylabel("X_{C} (Ω)",color=GRAY,fontsize=12)
ax.text(40,185,"C = 100 µF\nX_{C} minskar med f",fontsize=14,va="top"); save(F,"v40_02_s07_XC")
...
ax.plot([f0,f0],[0,97],color=RED,ls="--",lw=1.4); ax.text(f0+2,100,"resonans: X_{L} = X_{C}",color=RED,fontsize=13,va="center")
ax.text(100,72,"X_{L}",color=BLUE,fontsize=15); ax.text(25,85,"X_{C}",color=GREEN,fontsize=15)
...
def series_circuit(name,src,elems,extra=None,xl=(-1.4,4.5)):
    ...
    if extra: extra(ax)
    save(F,name)
series_circuit("v40_02_s11_ovn1","50 Hz",[("L","L = 0,10 H")],lambda ax:ax.text(1.7,0.8,"X_{L} = ?",ha="center",fontsize=16,color=RED))
series_circuit("v40_02_s13_ovn2","50 Hz",[("C","C = 100 µF")],lambda ax:ax.text(1.7,0.8,"X_{C} = ?",ha="center",fontsize=16,color=RED))
...
def ex(ax):
    ...
    for x,t in ((1.13,"U_{R} = ?"),(2.27,"U_{L} = ?")): ax.text(x,1.55,t,ha="center",va="top",color=RED,fontsize=14)
...
def ex(ax): ax.text(1.7,0.95,"50 Hz → 100 Hz",ha="center",fontsize=15,color=BLUE); ax.text(1.7,0.45,"X_{L} = ?   X_{C} = ?",ha="center",fontsize=15,color=RED)
...
F=fig(4.3,3.0); triangle(F,30,40,("R = 30 Ω","X_{L} = 40 Ω","|Z| = ?"),colors=(BLUE,GREEN,INK),rect=(0.06,0.04,0.9,0.92)); save(F,"v40_02_s15_ovn3")
F=fig(4.3,3.0); triangle(F,30,40,("R = 30 Ω","X_{L} = 40 Ω","|Z|"),colors=(BLUE,GREEN,INK),angle_label="φ = ?",rect=(0.06,0.04,0.9,0.92)); save(F,"v40_02_s26_ovn6")
# s21 RL and RC triangles
F=fig(4.3,4.4); ax=F.add_axes((0.02,0.02,0.96,0.96)); ax.set_aspect("equal"); ax.axis("off")
for y0,s_,lab,txt in ((5.3,1,"X_{L}","RL: strömmen släpar, φ > 0"),(3.3,-1,"X_{C}","RC: strömmen leder, φ < 0")):
...
comp("v40_03_s22_kompensering",3,4,4,("P = 3 kW oförändrad","Q_{L} =\n4 kvar","Q_{C} =\n4 kvar","S före = 5 kVA","S efter = P = 3 kVA"))
comp("v40_03_s28_ovn7",3,4,4,("P = 3 kW","Q =\n+4 kvar","Q_{C} = ?","S före"),h=3.0,exercise=True)
