...
def fit_box(shape, min_pt=16):
    ...
    return scale
def add_pic(slide,path,x,y,w,alt):
...
def process(src,dst,spec,figdir):
    ...
    for n,(kind,fname,alt) in spec.items():
        ...
        if kind=="ovn":
            ...
            tb.width=Inches(4.25); tb.height=Inches(2.85)
            sc=fit_box(tb)
            add_pic(s,f"{figdir}/{fname}.png",5.3,2.55,4.3,alt)
        else:
            scs=[]
            for sh in body:
                sh.width=Inches(4.35); scs.append(fit_box(sh))
            sc=min([x for x in scs if x] or [1])
            add_pic(s,f"{figdir}/{fname}.png",5.35,1.8,4.25,alt)
