...
ax.text(1.8, 1.8, "felströmsslinga " + S("Z", "s"), fontsize=12.5, color=ORANGE, weight="bold", ha="center")
ax.text(1.45, 1.45, "N och PE förbundna och", fontsize=11, color=GRAY, ha="center"); ax.text(1.45, 1.2, "jordade vid källan", fontsize=11, color=GRAY, ha="center")
ax.plot([0.8, 0.95, 0.95], [ys["N"], ys["N"], ys["PE"]], color=GREEN, lw=1.6); dot(ax, 0.95, ys["PE"], 0.045)
...
def it_fig(name, second=False):
    ...
    rbox(ax, 0.02, 1.3, 0.86, 0.55, "isolations-\nvakt", fs=10.5, fc=LBLUE); ax.plot([0.45, 0.45], [2.47, 1.8], color=GRAY, lw=1.5); ax.plot([0.45, 0.45], [1.35, 0.12], color=GRAY, lw=1.5)
...
z = np.linspace(0.3, 2.5, 200); ax.plot(z, 230 / z, color=BLUE, lw=2.6)
for zz in (0.575, 2.3): ax.plot([zz], [230 / zz], "o", color=ORANGE, ms=8); ax.plot([zz, zz], [0, 230 / zz], color=ORANGE, lw=1.2, ls=(0, (3, 3)))
ax.text(0.62, 420, "0,575 Ω: 400 A", fontsize=12, color=ORANGE); ax.text(2.3, 190, "2,3 Ω: 100 A", fontsize=12, color=ORANGE, ha="center")
ax.set_xlim(0.3, 2.5); ax.set_ylim(0, 800)
for s_ in ("top", "right"): ax.spines[s_].set_visible(False)
ax.set_xlabel(S("Z", "s") + " (Ω)", fontsize=12); ax.set_ylabel(S("I", "k") + " (A)", fontsize=12); ax.tick_params(labelsize=11)
ax.set_xticks([0.5, 1.0, 1.5, 2.0, 2.5]); ax.set_xticklabels(["0,5", "1,0", "1,5", "2,0", "2,5"])
F.text(0.56, 0.93, "Högre impedans ger lägre felström", ha="center", fontsize=12)
...
ax.plot([1.3, 3.0], [2.1, 2.1], color=INK, lw=1.8); ax.plot([1.3, 1.6, 1.6], [1.6, 1.6, 0.15], color=INK, lw=1.8)
...
ax.text(1.8, 2.3, S("U", "prov"), fontsize=13, color=BLUE)
