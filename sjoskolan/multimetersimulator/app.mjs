...
let saved = {done:[],records:[]}, storageOK = true;
try {
  const data = JSON.parse(localStorage.getItem(KEY) || 'null');
  if (data) saved = {done:Array.isArray(data.done) ? data.done.filter(id=>LESSONS.some(l=>l.id===id)) : [], records:Array.isArray(data.records) ? data.records.filter(r=>typeof r.lesson==='string'&&typeof r.time==='string').slice(-200) : []};
...
function startLesson(index, updateURL=true) {
  lessonIndex=index;stage=0;passed=false;free=false;choice=null;
  ...
  renderAll();feedback('Börja med uppgiften',l.circuit==='category'?'Läs situationen, välj ett alternativ och kontrollera ditt svar.':'Förutsäg mätvärdet. Välj sedan funktion och uttag, och anslut mätspetsarna.');
}
function renderLesson() {
  ...
  $('#task-heading').textContent=`Steg ${stage+1} av ${l.steps.length}`;
  ...
  $('#check').textContent=l.circuit==='category'?'Kontrollera svaret':'Kontrollera mätningen';
...
function circuitSVG() {
  ...
  if(state.circuit==='lamp') {
    ...
    body=wire('M 150 65 H 390 V 92 M 390 138 V 192 M 390 238 V 265 H 100 V 172 M 100 150 V 65 H 150')+
      ...
      label(100,217,'10 V DC')+label(475,122,'R1 · 1 MΩ')+label(475,220,'R2 · 1 MΩ');
  }
...
function renderCircuit() {
  ...
  if(state.circuit==='divider')controls+=`<label>Mätarens ingång<select id="input"><option value="10000000">10 MΩ</option><option value="1000000">1 MΩ</option></select></label>`;
...
function check() {
  ...
  const l=currentLesson(),step=currentStep(),m=l.circuit==='category'?null:measure(state);
  const correct=l.circuit==='category'?choice===step.correct:!state.trip&&step.test(state,m);
  if(correct){
    passed=true;
    saved.records.push({time:new Date().toISOString(),lesson:l.title,step:stage+1,mode:state.mode,jack:state.jack,red:state.red||'',black:state.black||'',reading:m?`${m.text} ${m.unit}`:step.choices[choice]});
    saved.records=saved.records.slice(-200);
    if(stage===l.steps.length-1&&!saved.done.includes(l.id))saved.done.push(l.id);
    save();renderLesson();feedback('Rätt kopplat och tolkat',step.why,'success');
  } else {
    const detail=l.circuit==='category'?(choice===null?'Välj ett alternativ först.':step.hint):(m.code!=='reading'&&m.code!=='open'?m.detail:`Avläsningen är ${m.text} ${m.unit}. Kontrollera mätfunktion, uttag och placering mot uppgiften. ${step.hint}`);
    feedback('Inte klart ännu',detail,'warning');
  }
}
function next() {
  if(stage<currentLesson().steps.length-1){stage++;passed=false;choice=null;$('#hint').open=false;renderLesson();feedback('Nästa steg',currentStep().task);}
...
$('#download-log').addEventListener('click',()=>{
  const rows=[['Tid (UTC)','Övning','Steg','Funktion','Uttag','Röd spets','Svart spets','Avläsning'],...saved.records.map(r=>[r.time,r.lesson,r.step,MODES[r.mode]||r.mode,r.jack==='v'?'V Ω':r.jack,r.red,r.black,r.reading])];
...
$('#clear-progress').addEventListener('click',()=>{
  ...
  saved={done:[],records:[]};save();confirmClear=false;$('#clear-progress').textContent='Nollställ framsteg';startLesson(0);
});
