...
export function visual(kind,width=760,progress=1){
 ...
 if(['calibration','meters','power','rms'].includes(kind)){
  ...
  const a=kind==='triangle'?40:1150,hojd=kind==='triangle'?30:Math.sqrt(2300**2-1150**2),scale=Math.min((w-105)/a,145/hojd),ox=(w-a*scale)/2,oy=210,rx=ox+a*scale,ry=oy-hojd*scale;
  b+=line(ox,oy,rx,oy,C.ink)+line(rx,oy,rx,ry,C.orange)+`<path d="M${ox},${oy} L${rx},${ry}" fill="none" stroke="${C.blue}" stroke-width="3"/>`;
  b+=text((ox+rx)/2,238,kind==='triangle'?'R = 40 Ω':'P = 1 150 W','middle')+text(rx-4,ry-15,kind==='triangle'?'X_{L} ≈ 30 Ω':'Q ≈ 1 992 var','end',C.orange)+text(ox,55,kind==='triangle'?'|Z| ≈ 50 Ω':'S = 2 300 VA','start',C.blue,20);
 }else{
