...
(() => {
  ...
  const LAB_TEXT = {
    sakerhet: { h: "Labb 0 Riskbedomning", body: "<p>Bocka checklistan. Huvudbrytare OFF innan koppling. SELV 12/24 V.</p><p>VG: motivera sakring i plus och varfor ohm aldrig anvands mot spanning.</p>" },
    ...
    ohm: { h: "Labb 2 Ohms lag", body: "<p>PSU 12 V och R100 eller R220. Mat U och I. R = U/I. Prova 8 V och 16 V.</p>" },
    serie: { h: "Labb 3 Serie och parallell", body: "<p>Tva lampor. Serie: delspanning. Parallell: stromdelning.</p>" },
    fall: { h: "Labb 4 Spanningsfall", body: "<p>PSU - KABEL - lampa. Mat U vid kalla och last. Rsladd = dU/I.</p>" },
    meter: { h: "Labb 5 Multimeter", body: "<p>V over last, A i serie, ohm bara spanningslost. Prova medvetet fel i ohm-lage.</p>" },
    hall: { h: "Labb 6 Hallkrets", body: "<p>24 V. PSU+ sakring stopp start A1. A2 till minus. 11-14 parallellt med start. Lampa fran 14.</p>" },
    fel: { h: "Labb 7 Felsokning", body: "<p>Plantera fel. Mat bakat fran last. Skriv orsak innan du byter del.</p>" },
    schema: { h: "Labb 8 Dokumentera", body: "<p>Bygg en fungerande krets. Skriv kalla, last, skydd och tre sladdars funktion.</p>" }
  };
  function storageKey(){ return "ellab-progress:" + (state.user.name||"anon").toLowerCase(); }
  ...
  function jackCenter(id){ const el=jackEl(id), board=$("#board"); if(!el||!board) return {x:0,y:0}; const a=el.querySelector(".hole").getBoundingClientRect(); const b=board.getBoundingClientRect(); return {x:a.left+a.width/2-b.left,y:a.top+a.height/2-b.top}; }
  function drawWires(){ const svg=$("#wires"); const b=$("#board").getBoundingClientRect(); svg.setAttribute("viewBox","0 0 "+b.width+" "+b.height); svg.innerHTML=state.wires.map((w)=>{ const p1=jackCenter(w.a), p2=jackCenter(w.b), mid=(p1.x+p2.x)/2; return '<path d="M '+p1.x+' '+p1.y+' C '+mid+' '+p1.y+', '+mid+' '+p2.y+', '+p2.x+' '+p2.y+'" fill="none" stroke="'+w.color+'" stroke-width="5" stroke-linecap="round"/>'; }).join(""); }
  ...
  function checkProtocol(){ const lab=state.labId; const out=$("#check-out"); const num=id=>{ const el=$(id); return el?parseFloat(el.value.replace(",",".")):NaN; }; let ok=[], bad=[];
    ...
    if(lab==="batteri"){ const u0=num("#p-u0"), ul=num("#p-ul"), i=num("#p-i"), ri=num("#p-ri"); if(u0>11.5&&u0<13.2) ok.push("U0 rimlig."); else bad.push("U0 runt 12.6 V."); if(ul&&u0&&ul<u0) ok.push("UL < U0."); else bad.push("UL ska vara lagre an U0."); if(i>1.2&&i<2.4) ok.push("Strom ok."); if(ri>0&&ri<1) ok.push("Ri ok."); }
  ...
  function bind(){ $("#lab-list").addEventListener("click",e=>{ const b=e.target.closest("[data-lab]"); if(!b) return; state.labId=b.dataset.lab; render(); }); $("#board").addEventListener("click",e=>{ const jack=e.target.closest("[data-jack]"); if(!jack) return; const mode=$("#probe-mode").value; if(mode==="wire") setPending(jack.dataset.jack); if(mode==="pos") probe("pos",jack.dataset.jack); if(mode==="com") probe("com",jack.dataset.jack); }); $("#sw-load").addEventListener("click",()=>{state.switches.load=!state.switches.load;render();}); $("#sw-main").addEventListener("click",()=>{state.switches.main=!state.switches.main;render();}); $("#sw-stop").addEventListener("click",()=>{state.switches.stop=!state.switches.stop;render();}); const start=$("#sw-start"); const down=()=>{state.switches.start=true;render();}; const up=()=>{state.switches.start=false;render();}; start.addEventListener("pointerdown",down); start.addEventListener("pointerup",up); start.addEventListener("pointerleave",up); $("#psu").addEventListener("input",e=>{state.psuV=+e.target.value;render();}); $("#btn-clear").addEventListener("click",clearWires); $("#btn-undo").addEventListener("click",removeLastWire); $("#btn-fuse").addEventListener("click",()=>{state.fuseBlown=false;render();}); $("#btn-weak").addEventListener("click",()=>{state.batteryWeak=!state.batteryWeak;render();}); $("#modes").addEventListener("click",e=>{ const m=e.target.closest("[data-mode]"); if(!m) return; state.dmm.mode=m.dataset.mode; render(); }); document.addEventListener("click",e=>{ if(e.target.id==="btn-check") checkProtocol(); }); $("#fault").addEventListener("change",e=>{state.fault=e.target.value||null;render();}); window.addEventListener("resize",drawWires); }
  function gate(){ const overlay=$("#gate"), who=$("#who"); if(state.user.name){ overlay.hidden=true; who.textContent=state.user.name+(state.user.group?" \u00b7 "+state.user.group:""); return;} overlay.hidden=false; }
  window.addEventListener("load",()=>{ loadUser(); bind(); $("#gate-form").addEventListener("submit",e=>{ e.preventDefault(); const name=$("#gate-name").value.trim(); const group=$("#gate-group").value.trim(); if(!name) return; state.user={name,group}; saveUser(); loadUser(); gate(); render(); }); $("#btn-logout").addEventListener("click",()=>{ state.user={name:"",group:""}; localStorage.removeItem("ellab-user"); gate(); }); gate(); render(); });
})();
