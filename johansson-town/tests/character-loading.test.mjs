...
test('delayed and failed character loads never show a placeholder or another identity',async()=>{
 ...
 try{
  ...
  failYuri=false;assert.equal(await preloadCharacter('Thuan'),true);characters.update(0);assert.equal(people[2].userData.character.isYuri,true,'Warm model mounts immediately without waiting behind scenery');
