...
test('delayed and failed character loads never show a placeholder or another identity',async()=>{
 ...
 globalThis.fetch=async input=>{const url=String(input.url||input);if(url.startsWith('blob:'))return original(input);
  ...
  if(url.includes('town-worker')){workerRequests++;await held;}
  if(url.includes('yuri-playful')&&failYuri)return new Response('',{status:503});
 ...
 try{
  ...
  for(const name of ['Kenji','Tetsuo','Yuri','Aya','Reiko']){
  ...
  const kenji=entries.find(e=>e.id==='resident:Kenji'),tetsuo=entries.find(e=>e.id==='resident:Tetsuo');
