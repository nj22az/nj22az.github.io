...
test('delayed and failed character loads never show a placeholder or another identity',async()=>{
 ...
 globalThis.fetch=async input=>{const url=String(input.url||input);if(url.startsWith('blob:'))return original(input);
  ...
  if(url.includes('characters/thuan/thuan.glb')&&failYuri)return new Response('',{status:503});
 ...
 try{
  ...
  for(const name of ['Harbour master','Bus driver','Thuan','Aya','Reiko']){
  ...
  const yuri=entries.find(e=>e.id==='resident:Thuan');assert.equal(await yuri.load(),false);assert.equal(people[2].children.length,0);assert.equal(people[2].userData.visualReady,false);
  failYuri=false;assert.equal(await preloadCharacter('Thuan'),true);characters.update(0);assert.equal(people[2].userData.character.isThuan,true,'Warm model mounts immediately without waiting behind scenery');
