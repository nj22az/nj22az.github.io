...
test('actual supplied scan is local, finite, textured and behind all northern approaches',async()=>{
 ...
 assert.equal(draws,2);assert.ok(tris<200000);assert.ok(Math.abs(box.min.z-36)<1e-4);assert.ok(box.max.x<25);assert.ok(Math.abs(box.min.y+.65)<1e-4);
...
test('barrier stops walking and swept running before, after and without a model load',async()=>{
 ...
 for(let x=-6.5;x<=6.5;x+=.5){assert.equal(blocked(x,32),false);assert.equal(blocked(x,34.4),true);assert.ok(sweepFraction({x,z:32},{x,z:40},blocked)<.3);}
 ...
 assert.equal(failed.seaCave.status,'fallback');assert.ok(failed.group.getObjectByName('Sea cave loading fallback'));assert.ok(failed.colliders.some(c=>circleHitsRect(0,34.4,.32,c)));
});
