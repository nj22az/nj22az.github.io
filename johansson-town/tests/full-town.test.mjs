...
test('Complete supplied overworld preserves gameplay, reachable destinations and touch sprint',async()=>{
  try {
    ...
    const {preloadFullTown}=await import('../src/world/full-town.js?snappy=1');
    ...
    assert.equal(api.world.group.getObjectByName('Thuan house lantern'),undefined);
    ...
    assert.ok(api.world.group.getObjectByName('Thuan canal house'));
    ...
    const yuriHouse=api.world.group.getObjectByName('Thuan canal house');
    assert.ok(yuriHouse.position.x<-6.5&&yuriHouse.position.x>-7.3,'Thuan’s house sits on the west canal bank');
    assert.ok(yuriHouse.position.z>11&&yuriHouse.position.z<12,'Thuan’s house faces the north canal boardwalk');
    assert.ok(yuriHouse.scale.y>.9,'Thuan’s house keeps a two-storey height');
    assert.ok(yuriHouse.getObjectByName('Thuan house exterior')||yuriHouse.getObjectByName('Thuan house noren'));
    ...
    const {createNavigation}=await import('../src/people/navmesh.js?snappy=1');
    const {townBoundsBlocked,circleHitsRect}=await import('../physics.js?snappy=1');
