...
test('eight low-poly identities are distinct, retain shared body buffers and keep original Yuri only as a small static figurine',async()=>{
 ...
 try{
  await preloadModels();assert.equal(requests.length,7);assert.ok(requests.every(url=>!url.includes('yuri-playful')&&!url.includes('vroid')));
  const scene=new THREE.Scene(),models=createLocalCharacters(),signatures=new Set(),actors=[];
  for(const profile of RESIDENTS.filter(p=>!['Aya','Reiko'].includes(p.name))){
  ...
  assert.equal(signatures.size,8);const yuri=actors.find(a=>a.isYuri);assert.equal(yuri.height,1.64);assert.ok(yuri.model.getObjectByName('resident-ribbon-apron'));
  ...
  const bounds=new THREE.Box3().setFromObject(figure);assert.ok(Math.abs(bounds.max.y-bounds.min.y-.30)<.001);assert.equal(figure.userData.figurine,true);assert.equal(models.actors.length,8);
