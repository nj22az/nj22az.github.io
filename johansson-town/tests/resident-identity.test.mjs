...
test('all named low-poly identities are distinct, retain shared body buffers and keep original Yuri only as a small static figurine',async()=>{
 ...
 try{
  ...
  assert.equal(signatures.size,cast.length);const yuri=actors.find(a=>a.isYuri);assert.equal(yuri.height,1.64);assert.ok(yuri.model.getObjectByName('resident-ribbon'));
  const nao=actors.find(a=>a.entity.userData.name==='Nao');let ys,ns;for(const [actor,id] of [[yuri,'y'],[nao,'n']])actor.model.traverse(o=>{if(o.isSkinnedMesh&&!o.userData.facialFeatures){if(id==='y')ys=o;else ns=o;}});assert.notEqual(ys.geometry.attributes.position,ns.geometry.attributes.position,'Yuri can blink without modifying Nao’s geometry');assert.deepEqual(ys.geometry.attributes.position.array,ns.geometry.attributes.position.array);assert.notDeepEqual(Array.from(ys.geometry.attributes.color.array),Array.from(ns.geometry.attributes.color.array));
