...
test('six local skinned bodies parse, animate and clone independently',async()=>{
 ...
 try{
  assert.deepEqual(await preloadModels(),{ready:6,total:6});
  ...
  for(const name of ['Johansson','Aiko','Kenji','Mrs Sato','Hana','Kenta']){
   ...
   let skins=0;actor.model.traverse(o=>{if(o.isSkinnedMesh){skins++;assert.equal(Array.isArray(o.material),false);assert.equal(o.geometry.groups.length,0);assert.ok(o.skeleton.bones.length>20);}});assert.equal(skins,name==='Kenji'?6:1);
  ...
  const kenji=actors[2],point=new THREE.Vector3();
  for(const action of kenji.actions.values()){
   ...
   for(const fraction of [0,.25,.5,.75,1]){
    ...
    kenji.model.traverse(mesh=>{if(!mesh.isSkinnedMesh)return;mesh.skeleton.update();assert.ok(mesh.material.map,'Keep authored texture maps');
