...
test('Japanese Town kit replaces all shops and homes with bounded shared geometry and clear routes',async()=>{
 ...
 try{
  ...
  const batches=[];world.group.traverse(o=>{if(o.name==='japanese-homes')batches.push(o);});assert.equal(batches.reduce((n,o)=>n+o.count,0),22);assert.ok(batches.length<=8,'Homes share geometry and are batched by nearby blocks');assert.ok(batches.every(o=>o.material===batches[0].material));
