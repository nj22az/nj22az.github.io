...
test('Thuan sits and stands against the rendered chair with planted feet, clear limbs and continuous movement',()=>setup(()=>{
 ...
 const phases=new Set(),feet=['LeftToeBase','RightToeBase'].map(n=>rigBone(actor.model,n)),bones=['Hips','Head','LeftHand','RightHand'].map(n=>rigBone(actor.model,n)),last=new Map(),anchors=new Map();
 // Every visible skinned part: the Meshy rig is one mesh, the MakeHuman one is dressed in several.
 const meshes=[];actor.model.traverse(o=>{if(o.isSkinnedMesh&&o.visible)meshes.push(o);});let previousPhase='',maxJointStep=0,jumpDetail='',largestSeatedLean=0;const p=new T.Vector3();
 for(let frame=1;frame<=55*60;frame++){
  ...
  if(seated){
   assert.ok(Math.abs(clerk.rotation.y-seat.yaw)<.001);const head=rigBone(actor.model,'Head').getWorldPosition(new T.Vector3()),hip=rigBone(actor.model,'Hips').getWorldPosition(new T.Vector3());largestSeatedLean=Math.max(largestSeatedLean,Math.hypot(head.x-hip.x,head.z-hip.z));
  }
  if((transition||seated||service.phase==='break-turn')&&frame%12===0){
   ...
   for(const mesh of meshes){mesh.skeleton.update();for(let i=0;i<mesh.geometry.attributes.position.count;i++){
    ...
    assert.ok(!solids.some(box=>box.containsPoint(p)),`Body penetrates the rendered chair during ${service.phase} at ${frame/60}: ${mesh.name} ${p.toArray()}`);
   }}
 ...
 assert.ok(maxJointStep<.045,'No pose snap at the chair transition: '+maxJointStep+' '+jumpDetail);assert.ok(largestSeatedLean<.08,'Quiet upright idle throughout the loop: '+largestSeatedLean);service.dispose();assert.equal(clerk.userData.chairBlend,undefined);assert.equal(clerk.userData.floorHeight,undefined);
}));
