...
export function createSakuraShop({world,scene,state,ledger,register,action,exit,getMinutes,getPlayerPosition,isInside,onBorrow=()=>{},getRain=()=>false,save=()=>{}}){
 ...
 service=createRetailClerk({person,room:group,layout,collides:blocked,getWork:retail.work,completeWork:job=>{retail.complete(job);save();},cancelWork:retail.cancelWork,
 ...
 return {group,colliders,service,retail,display,blocked,layout,ready:display.ready,
  ...
  update(dt){residents.sync(getMinutes(),dt);retail.update(dt);service.update(dt);display.updateStock(state.sakura.stock);},
 };
}
