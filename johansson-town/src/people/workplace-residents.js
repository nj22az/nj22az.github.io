...
export function createWorkplaceResidents({world,parent,getTargets,collides,getPlayerPosition,getState,ledger,onBorrow=()=>{},getEntrance=()=>[0,0,5.2]}){
 const borrowed=new Map(),routes=new Map();let site=null,interactions=null,navigation=null,walker=null,clock=0;
 function restore(person){
  ...
  for(const key of ['inWorkplace','indoors','usingTownObject','socialPose','seatHeight','heldItem','roomTransition'])delete g.userData[key];if(residentPlan(person.profile,clock).place==='work'&&person.profile.workSite){g.userData.indoors='work';g.visible=false;}borrowed.delete(person);routes.delete(person);
 }
 function update(dt,minutes,rain){
  clock=minutes;if(!site)return;
  const worker=world.people.find(p=>p.profile.name===WORKERS[site.id]&&(!p.profile.workSite||p.profile.workSite===site.id));
  for(const person of [...borrowed.keys()])if(residentPlan(person.profile,minutes,rain).place!=='work'||person.profile.name==='Kenji'&&getState().kenjiEscort==='walking'){
   interactions?.release(person,minutes);person.g.userData.roomTransition=true;person.g.userData.activity='leaving work';
   if(walker.move(person,getEntrance(),dt))restore(person);
  }
  if(worker&&!borrowed.has(worker)&&atDestination(worker,'work',worker.profile.work)&&residentPlan(worker.profile,minutes,rain).place==='work'&&!(worker.profile.name==='Kenji'&&getState().kenjiEscort==='walking')){
   ...
   if(spawn){const g=worker.g;onBorrow(worker,minutes);borrowed.set(worker,{parent:g.parent,position:g.position.clone(),rotation:g.quaternion.clone(),inside:g.userData.hit.inside,stand:spawn,arriving:true});parent.add(g);g.position.set(...getEntrance());g.userData.hit.inside=true;g.userData.inWorkplace=site.id;g.userData.indoors=site.id;g.visible=true;}
  }
  for(const [person,saved] of borrowed){
   const g=person.g;if(g.userData.roomTransition&&!saved.arriving)continue;if(residentPlan(person.profile,minutes,rain).place!=='work')continue;
   if(saved.arriving){g.userData.roomTransition=true;g.userData.activity='walking to work';if(!walker.move(person,[saved.stand[0],0,saved.stand[1]],dt))continue;saved.arriving=false;delete g.userData.roomTransition;}
   const base={place:'work',target:saved.stand,activity:'working at '+site.title},plan=interactions.plan(person,base,minutes,rain,dt);
 ...
 return {enter(next,minutes){leave();if(!WORKERS[next.id])return;site=next;walker=createRoomWalk(collides);
