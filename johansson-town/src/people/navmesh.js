import {MAP_BOUNDS} from '../world/layout.js';
...
export function createNavigation(blocked,{step=1,bounds=MAP_BOUNDS}={}){
  ...
  const edgeClear=(a,b)=>{const k=key(...a)+'>'+key(...b);if(!edges.has(k)){const dx=b[0]-a[0],dz=b[1]-a[1],n=Math.ceil(Math.hypot(dx,dz)*step/.15);let safe=true;for(let i=1;i<n;i++)if(blocked((a[0]+dx*i/n)*step,(a[1]+dz*i/n)*step,.32)){safe=false;break;}edges.set(k,safe);}return edges.get(k);};
