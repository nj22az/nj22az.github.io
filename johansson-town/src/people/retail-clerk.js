...
export function createRetailClerk({person,room,layout,collides,getWork,completeWork,cancelWork,accessShelf=()=>{},isBlocked=()=>false}){
 ...
 const finish=()=>{const restocked=job?.type==='restock';if(job)completeWork(job);job=null;box.visible=restocked&&--cartonStops>0;delete clerk.userData.shopReach;phase=restocked?'stock-next':'return';};
 return {get phase(){return phase;},get order(){return null;},cancel(){},occupied(){return false;},prepareToLeave(){if(phase==='counter')return true;if(job){cancelWork(job);job=null;}box.visible=false;phase='return';return false;},
  update(dt){
   ...
   else if(phase==='stock-collect'){if((timer-=dt)<=0){box.visible=true;cartonStops=3;phase='stock-carry';}}
   ...
   else if(phase==='stock-place'){accessShelf(job.item);if((timer-=dt)<=0)finish();}
