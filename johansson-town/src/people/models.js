...
const SOURCES=['protagonist','suit','yui','yuri-playful',...PROFILES.map(p=>p.model)];
...
export function preloadModels({onProgress}={}){
  ...
  pending=Promise.allSettled(SOURCES.map(async id=>{
    ...
    try{
      const response=await fetch(assetURL(id==='protagonist'?'characters/protagonist/johansson.glb':id.startsWith('resident-')?'characters/living/'+id+'.glb':['kenji','yui','yuri-playful','protagonist'].includes(id)?'characters/realistic/'+id+'.glb'+(id==='yuri-playful'?'?yuri-rig-2':''):'characters/residents/town-'+id+'.glb'),{signal:abort.signal});
      if(!response.ok)throw Error('Local character unavailable: '+id);
      const data=await response.arrayBuffer();
      const gltf=await loader.parseAsync(data,'');gltf.scene.traverse(o=>{if(o.isSkinnedMesh&&!id.startsWith('resident-')&&!['kenji','yui','yuri-playful','protagonist'].includes(id)){smoothCharacterNormals(o.geometry);o.material.flatShading=false;o.material.roughness=.78;o.material.dithering=true;}});if(id==='yuri-playful')gltf.animations=prepareYuriAnimations(gltf);if(id==='protagonist')gltf.animations=prepareProtagonistAnimations(gltf);loaded.set(id,gltf);
...
function sourceFor(name,profile){
  ...
  if(name==='player'||name==='Johansson')return loaded.has('protagonist')?'protagonist':'suit';
...
export function createLocalCharacters({shadows=false}={}){
  ...
  function attach(entity,name,height){
    ...
    model.traverse(o=>{if(o.isMesh){o.castShadow=shadows;o.receiveShadow=shadows;o.frustumCulled=false;if(!source.startsWith('resident-')&&!['kenji','yui','yuri-playful','protagonist'].includes(source))dressCharacter(o,profile?.top);}});
    for(const child of entity.children)child.visible=false;
    entity.add(model);entity.userData.visualSource=source==='protagonist'?'User-supplied Meshy · Johansson':source.startsWith('resident-')?'Original Blender living cast · '+name:source==='yuri-playful'?'User-supplied Meshy · Yuri':['kenji','yui','yuri-playful','protagonist'].includes(source)?'Blender / MakeHuman · '+name:'Quaternius / '+source;
    ...
    const actor={cup,entity,model,mixer,actions,current:null,last:entity.position.clone(),gestureTime:0,speed:0,isYuri:source==='yuri-playful',isPlayer:source==='protagonist'};
    byEntity.set(entity,actor);actors.push(actor);return actor;
  }
  function update(dt){
    for(const actor of actors){const {entity,mixer,actions}=actor;
      if(!entity.visible&&!actor.isPlayer){actor.last.copy(entity.position);continue;}
