import {createAdvertisingBillboard} from './world/advertising-billboard.js';
...
let inspector=null,content=null,castAI=null,hands=null,storeService=null,venueService=null,seated=false,parkSeat=null,touchRunning=false;
...
const advertisingBillboard=createAdvertisingBillboard({parent:town,position:FULL_TOWN.active?[7.02,6.4,2.15]:[konbini.side*6,6.4,konbini.z],yaw:FULL_TOWN.active?Math.PI:-konbini.side*Math.PI/2});
...
const workplaceResidents=createWorkplaceResidents({world,parent:scene,getEntrance:()=>activeRoomLayout?.spawn||[0,0,5.2],getTargets:()=>interactables,collides:environmentBlocked,getPlayerPosition:()=>player.position,getState:()=>activities.state,ledger:residentLedger,onBorrow:npcActivities.release});
...
const izakayaGuests=createIzakayaGuests({world,parent:scene,collides:environmentBlocked,getRain:()=>weather,getState:()=>activities.state,onBorrow:npcActivities.release,getYuri:()=>{const g=ensureYuri();if(!interactables.includes(g))reg(g,'Catch up with Yuri',()=>activities.action('resident','Yuri'),true);return g;}});
const ramenGuests=createIndoorResidents({world,parent:scene,collides:environmentBlocked,getRain:()=>weather,place:'ramen',onBorrow:npcActivities.release,getState:()=>activities.state}),marketClerk=createIndoorResidents({world,parent:scene,collides:environmentBlocked,getRain:()=>weather,place:'market',onBorrow:npcActivities.release,getState:()=>activities.state,getPlayerSeat:()=>parkSeat?.storeSeatId}),homeGuests=createHomeResidents({world,parent:scene,collides:environmentBlocked,onBorrow:npcActivities.release,getRain:()=>weather});
...
function addRoomProps(s){
  ...
  if(s.id==='izakaya'){buildIzakayaRoom({room,box,reg,collider:roomCollider,action:activities.action,exit:leaveRoom,signTexture:signTex});izakayaGuests.sync(minutes);startVenueService('izakaya');const light=new THREE.HemisphereLight(0xffdfaa,0x886d5f,1.6);room.add(light);return;}
...
function runStabilityChecks(){const failures=[];if(!townBoundsBlocked(160,0,PLAYER_RADIUS))failures.push('town edge');if(!roomBoundsBlocked(5.5,0,PLAYER_RADIUS))failures.push('room edge');if(world.colliders.length<20)failures.push('world collider coverage');if((world.quality?.streetInteractions||0)<8)failures.push('street interaction coverage');for(const [id,p] of doors){const site=SITES.find(s=>s.id===id)||(world.landmarks||[]).find(s=>s.id===id);const facing=id==='warehouse'||homeOwner(site)?site?.entryFacing:null;const ex=Number.isFinite(facing)?p.x+Math.sin(facing)*.7:p.x,ez=Number.isFinite(facing)?p.z+Math.cos(facing)*.7:p.z+(id==='izakaya'?-.7:.7);if(environmentBlocked(p.x,p.z,PLAYER_RADIUS)||(!FULL_TOWN.active&&environmentBlocked(ex,ez,PLAYER_RADIUS)))failures.push(`door spawn ${id}`);}window.__JOHANSSON_STABILITY__={ok:failures.length===0,failures,colliders:world.colliders.length,characterCount:world.people.length+1,streetInteractions:world.quality?.streetInteractions||0,renderDpr:renderer.getPixelRatio(),toneMapping:'AgX',shadows};if(failures.length)console.error('Johansson Town stability checks failed',failures);else console.info('Johansson Town stability checks passed',window.__JOHANSSON_STABILITY__)}
