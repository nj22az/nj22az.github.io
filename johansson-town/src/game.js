import {createNeighbourChats,createChatBubble,clearChatLine} from './people/neighbour-chats.js';
import {createIndoorResidents} from './people/indoor-residents.js';
import {buildSuppliedRoom,suppliedRoomBoundsBlocked} from './world/supplied-rooms.js';
...
import { createCharacters } from './people/characters.js?vroid-5';
...
let inspector=null,content=null,castAI=null,hands=null,seated=false,parkSeat=null,touchRunning=false;
...
const ramenGuests=createIndoorResidents({world,parent:scene,place:'ramen'}),marketClerk=createIndoorResidents({world,parent:scene,place:'market'});
...
function addRoomProps(s){
  if(activeRoomLayout){if(s.id==='ramen')ramenGuests.sync(minutes);return;}
...
function enterRoom(s){parkSeat=null;seated=false;if(world.isOpen&&!world.isOpen(s,minutes)){say('準備中 · Closed. Opens at '+(s.opens||'09:00')+'.',4);return;}activities.close();activities.visit(s.id);current=s;roomShell(s);addRoomProps(s);room.traverse(o=>{if(!o.isMesh||o.userData.sharedAsset)return;const b=o.geometry?.parameters;if(b?.height<.25&&o.position.y>3.8||o.position.z>6&&o.position.y>1||o.position.x>6&&o.position.y>1){o.userData.cutaway=true;o.layers.set(0);}});player.position.set(...(activeRoomLayout?.spawn||[0,0,4.3]));yaw=0;$('#exitRoomButton').classList.remove('hidden');syncView();active=null;$('#place').textContent=s.title.toUpperCase();$('#placeSub').textContent=`${s.jp} · ${s.sub}`;$('#timeText').textContent=s.line;say(s.id==='stepwise'&&activeRoomLayout?'The workshop door closes. Something here does not belong to the street.':`${s.title} · Explore the room. Tap objects nearby to examine them.`,s.id==='stepwise'?5:3);camera.position.copy(player.position);camera.position.y+=1.7;}
function leaveRoom(){if(!current)return;neighbourChats.cancel();chatBubble.hide();inspector?.close();activities.close();resetInput();$('#directory').classList.add('hidden');$('#exitRoomButton').classList.add('hidden');izakayaGuests.restore();ramenGuests.restore();marketClerk.restore();seated=false;active=null;const s=current;current=null;syncView();room.visible=false;town.visible=true;if(s){player.position.copy(doors.get(s.id));player.position.z+=.6}$('#place').textContent='JOHANSSON TOWN';$('#placeSub').textContent='ヨハンソン町 · HARBOUR DISTRICT';$('#timeText').textContent='Shops are open.';say('Johansson Street',1.5)}
...
function doInteract(){if(seated){if(parkSeat)player.position.set(...parkSeat.stand);parkSeat=null;seated=false;say('You stand up.',2);return;}if(inspector?.active)return;if(!activities.paused&&$('#directory').classList.contains('hidden')){interaction();active?.fn?.();}}
...
function runStabilityChecks(){const failures=[];if(!townBoundsBlocked(160,0,PLAYER_RADIUS))failures.push('town edge');if(!roomBoundsBlocked(5.5,0,PLAYER_RADIUS))failures.push('room edge');if(world.colliders.length<20)failures.push('world collider coverage');if((world.quality?.streetInteractions||0)<8)failures.push('street interaction coverage');for(const [id,p] of doors)if(environmentBlocked(p.x,p.z,PLAYER_RADIUS)||environmentBlocked(p.x,p.z+(id==='izakaya'?-.7:.7),PLAYER_RADIUS))failures.push(`door spawn ${id}`);window.__JOHANSSON_STABILITY__={ok:failures.length===0,failures,colliders:world.colliders.length,characterCount:world.people.length+1,streetInteractions:world.quality?.streetInteractions||0,renderDpr:renderer.getPixelRatio(),toneMapping:'AgX',shadows};if(failures.length)console.error('Johansson Town stability checks failed',failures);else console.info('Johansson Town stability checks passed',window.__JOHANSSON_STABILITY__)}
...
function simulate(frameDt){
  ...
  while(accumulator>=SIM_STEP){
    ...
    if(current&&['izakaya','market','ramen'].includes(current.id)&&!world.isOpen(current,minutes)){const name=current.title;leaveRoom();say(name+' is closing. Time to head home.',4);}
    if(current?.id==='izakaya')izakayaGuests.sync(minutes);
    if(current?.id==='market')marketClerk.sync(minutes);
...
function loop(){requestAnimationFrame(loop);updateContextControls();const frameDt=Math.min(clock.getDelta(),MAX_FRAME_DT);if(started&&!document.hidden){const paused=inspector?.active||activities.paused||!$('#directory').classList.contains('hidden');if(!paused){const before=player.position.clone();simulate(frameDt);if(player.position.distanceTo(before)>.01&&(stepTick+=frameDt)>.42){activities.footstep(current?'wood':routeAt(player.position.x,player.position.z)?.surface||'stone');stepTick=0;}interaction();}else{neighbourChats.cancel();chatBubble.hide();accumulator=0;resetInput();$('#prompt').classList.remove('on');}townAudio.update({player:player.position,yaw,minutes,rain:weather,inside:!!current,station:activities.state.radioStation||0,paused});$('#clock').textContent=fmt(minutes);setTime();hands?.update(paused?0:frameDt);if(!inspector?.active){characters?.update(frameDt);castAI?.pose(frameDt);}if(!paused)chatBubble.render(neighbourChats.current);if((mapTick+=frameDt)>.15){drawMap();mapTick=0;}if(subtitleTimer>0&&(subtitleTimer-=frameDt)<=0)$('#subtitle').classList.remove('on');if(inspector?.active)inspector.render(frameDt);else renderer.render(scene,camera)}else{neighbourChats.cancel();chatBubble.hide();}}renderer.render(scene,camera);loop();
