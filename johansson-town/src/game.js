...
const workplaceResidents=createWorkplaceResidents({world,parent:scene,getTargets:()=>interactables,collides:environmentBlocked,getPlayerPosition:()=>player.position,getState:()=>activities.state,ledger:residentLedger,onBorrow:npcActivities.release});
...
const izakayaGuests=createIzakayaGuests({world,parent:scene,getState:()=>activities.state,onBorrow:npcActivities.release,getYuri:()=>{const g=ensureYuri();if(!interactables.includes(g))reg(g,'Catch up with Yuri',()=>activities.action('resident','Yuri'),true);return g;}});
const ramenGuests=createIndoorResidents({world,parent:scene,place:'ramen',onBorrow:npcActivities.release,getState:()=>activities.state}),marketClerk=createIndoorResidents({world,parent:scene,place:'market',onBorrow:npcActivities.release,getState:()=>activities.state,getPlayerSeat:()=>parkSeat?.storeSeatId}),homeGuests=createIndoorResidents({world,parent:scene,place:'home',onBorrow:npcActivities.release});
