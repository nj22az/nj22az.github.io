...
function doInteract(){if(roomLoading)return;if(seated){if(parkSeat?.stand)player.position.set(...parkSeat.stand);parkSeat=null;seated=false;unstuckPlayer();say('You stand up.',2);return;}if(inspector?.active)return;if(!activities.paused&&$('#directory').classList.contains('hidden')){interaction();active?.fn?.();}}
