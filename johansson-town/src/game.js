...
function leaveRoom(){if(!current)return;neighbourChats.cancel();chatBubble.hide();inspector?.close();activities.close();resetInput();$('#directory').classList.add('hidden');$('#exitRoomButton').classList.add('hidden');izakayaGuests.restore();ramenGuests.restore();marketClerk.restore();seated=false;active=null;const s=current;current=null;syncView();room.visible=false;town.visible=true;if(s)placeAtEntrance(s,true);$('#place').textContent='JOHANSSON TOWN';$('#placeSub').textContent='ヨハンソン町 · HARBOUR DISTRICT';$('#timeText').textContent='Shops are open.';say('Johansson Street',1.5)}
...
function overlapsResident(x,z){return world.people.some(p=>p.g.visible&&(!current||p.g.userData.inIzakaya)&&circleHitsCircle(x,z,PLAYER_RADIUS,p.g.position.x,p.g.position.z,NPC_RADIUS));}
function residentBlocked(x,z){if(current?.id==='izakaya'&&storeClerk?.visible&&circleHitsCircle(x,z,PLAYER_RADIUS,storeClerk.position.x,storeClerk.position.z,NPC_RADIUS))return true;return world.people.some(p=>{if(!p.g.visible||(current&&!p.g.userData.inIzakaya))return false;if(!current&&inEntrance(p.g.position.x,p.g.position.z))return false;return circleHitsCircle(x,z,PLAYER_RADIUS,p.g.position.x,p.g.position.z,NPC_RADIUS);});}
...
function placeAtEntrance(s,leave=false){
 ...
 const tryAt=(x,z)=>!environmentBlocked(x,z)&&!overlapsResident(x,z);
 let x=x0,z=z0;
 if(leave)for(let d=0;d<=2.8&&!tryAt(x,z);d+=.2){x=x0+fwX*d;z=z0+fwZ*d;}
 if(!tryAt(x,z))for(let r=leave?.4:.2;r<=3;r+=.25){let found=false;for(let i=0;i<16;i++){const a=i/16*Math.PI*2,px=x0+Math.cos(a)*r,pz=z0+Math.sin(a)*r;if(tryAt(px,pz)){x=px;z=pz;found=true;break;}}if(found)break;}
