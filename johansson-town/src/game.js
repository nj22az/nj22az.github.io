...
function leaveRoom(){if(!current)return;neighbourChats.cancel();chatBubble.hide();inspector?.close();activities.close();resetInput();$('#directory').classList.add('hidden');$('#exitRoomButton').classList.add('hidden');izakayaGuests.restore();ramenGuests.restore();marketClerk.restore();seated=false;active=null;const s=current;current=null;syncView();room.visible=false;town.visible=true;if(s){if(s.exitPosition){player.position.set(...s.exitPosition);yaw=s.entryFacing+Math.PI;}else{player.position.copy(doors.get(s.id));player.position.z+=.6}}$('#place').textContent='JOHANSSON TOWN';$('#placeSub').textContent='ヨハンソン町 · HARBOUR DISTRICT';$('#timeText').textContent='Shops are open.';say('Johansson Street',1.5)}
...
function visitPlace(s){
 ...
 if(current)leaveRoom();const p=doors.get(s.id);player.position.set(p.x,groundHeight(p.x,p.z),p.z+(s.exitPosition?0:s.id==='izakaya'?-1.2:.7));yaw=s.entryFacing??(s.id==='izakaya'?Math.PI:s.side?-s.side*Math.PI/2:0);resetInput();toggleDir(false);navigationTarget=null;drawMap();say(s.title+' · You took a familiar shortcut.',5);return true;
}
