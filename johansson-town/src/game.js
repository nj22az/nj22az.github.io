...
let conversationName=null,conversationCamera=null;
let current=null,active=null,started=false,yaw=0,pitch=-.05,cam=3,minutes=1002,subtitleTimer=0,weather=false,activities=null,timePreset=0,characters=null;
...
function enterRoom(s){seated=false;if(world.isOpen&&!world.isOpen(s,minutes)){say('準備中 · Closed. Opens at '+(s.opens||'09:00')+'.',4);return;}activities.close();activities.visit(s.id);current=s;roomShell(s);addRoomProps(s);room.traverse(o=>{if(!o.isMesh||o.userData.sharedAsset)return;const b=o.geometry?.parameters;if(b?.height<.25&&o.position.y>3.8||o.position.z>6&&o.position.y>1||o.position.x>6&&o.position.y>1)o.layers.set(31);});player.position.set(0,0,4.3);yaw=0;document.body.classList.add('diorama');$('#exitRoomButton').classList.remove('hidden');camera.fov=38;camera.updateProjectionMatrix();active=null;$('#place').textContent=s.title.toUpperCase();$('#placeSub').textContent=`${s.jp} · ${s.sub}`;$('#timeText').textContent=s.line;say(`${s.title} · Explore the miniature room. Tap objects nearby to examine them.`,3);{const d=Math.max(18,13/Math.max(.42,camera.aspect));camera.position.set(8,d*.72,d);camera.lookAt(0,.6,-.4);}}
function leaveRoom(){if(!current)return;inspector?.close();activities.close();resetInput();$('#directory').classList.add('hidden');$('#exitRoomButton').classList.add('hidden');izakayaGuests.restore();document.body.classList.remove('diorama');camera.fov=65;camera.updateProjectionMatrix();if(storeClerk)storeClerk.visible=false;seated=false;active=null;const s=current;current=null;room.visible=false;town.visible=true;if(s){player.position.copy(doors.get(s.id));player.position.z+=.6}$('#place').textContent='JOHANSSON TOWN';$('#placeSub').textContent='ヨハンソン町 · HARBOUR DISTRICT';$('#timeText').textContent='Shops are open.';say('Johansson Street',1.5)}
...
function visitPlace(s){if(current)leaveRoom();const p=doors.get(s.id);player.position.set(p.x,groundHeight(p.x,p.z),p.z+(s.id==='izakaya'?-1.2:.7));yaw=s.id==='izakaya'?Math.PI:s.side?-s.side*Math.PI/2:0;resetInput();toggleDir(false);say(s.id==='izakaya'?'Minato Izakaya · The lantern entrance is ahead. Open 16:00–23:30.':s.title,5);}
...
$('#directoryGrid').innerHTML=SITES.map(s=>`<button class="dir-item" data-id="${s.id}"><b>${s.title}</b><span>${s.sub}</span></button>`).join('');document.querySelectorAll('.dir-item').forEach(b=>b.onclick=()=>{visitPlace(SITES.find(x=>x.id===b.dataset.id))});$('#directoryButton').onclick=()=>toggleDir(true);$('#closeDirectory').onclick=()=>toggleDir(false);
...
window.__JOHANSSON_CAMERA_MODE__='first';started=true;$('#start').classList.add('hidden');$('#hud').classList.remove('hidden');window.__JOHANSSON_RUNNING__=true;camera.position.set(0,3.5,50);say('Aiko is by the book display ahead. The harbour office keeps a blue-taped folio towards the water.',5);runStabilityChecks();
...
const mapCanvas=$('#minimap'),mapCtx=mapCanvas.getContext('2d');function drawMap(){drawTownMap(mapCtx,mapCanvas.width,mapCanvas.height,{sites:SITES,people:world.people,player:current?doors.get(current.id):player.position,yaw,visited:activities.state.visited});}
...
function simulate(frameDt){
  ...
  while(accumulator>=SIM_STEP){
    ...
    if(current?.id==='izakaya')izakayaGuests.sync(minutes);if(!current){world.update(dt,elapsed,daylight(minutes),minutes);castAI?.update(dt,minutes,weather);world.beats?.update(dt,elapsed,minutes);}
