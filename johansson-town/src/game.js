...
const ramenGuests=createIndoorResidents({world,parent:scene,place:'ramen'}),marketClerk=createIndoorResidents({world,parent:scene,place:'market'}),homeGuests=createIndoorResidents({world,parent:scene,place:'home'});
...
function leaveRoom(){if(!current)return;neighbourChats.cancel();chatBubble.hide();inspector?.close();activities.close();resetInput();$('#directory').classList.add('hidden');$('#exitRoomButton').classList.add('hidden');izakayaGuests.restore();ramenGuests.restore();marketClerk.restore();homeGuests.restore();seated=false;active=null;const s=current;current=null;syncView();room.visible=false;town.visible=true;if(s)placeAtEntrance(s,true);$('#place').textContent='JOHANSSON TOWN';$('#placeSub').textContent='ヨハンソン町 · HARBOUR DISTRICT';$('#timeText').textContent='Shops are open.';say('Johansson Street',1.5)}
