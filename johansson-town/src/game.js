...
import { createActivities } from '../activities.js?exploration-1';
...
import { createCharacters } from './people/characters.js?protagonist-1';
...
let inspector=null,content=null,castAI=null,hands=null,seated=false,touchRunning=false;
let conversationName=null,conversationCamera=null,navigationTarget=null,cameraNeedsSnap=true;
let current=null,active=null,started=false,yaw=0,pitch=-.05,cam=0,minutes=1002,subtitleTimer=0,weather=false,activities=null,timePreset=0,characters=null;
const keys={},clock=new THREE.Clock(),moveTouch={id:null,cx:0,cy:0,x:0,y:0},lookTouch={id:null,x:0,y:0};
const PLAYER_RADIUS=.28,NPC_RADIUS=.35,CAMERA_RADIUS=.1,MAX_FRAME_DT=.1,SIM_STEP=1/60;
...
characters=createCharacters({mobile,shadows,canJump:()=>!seated,isBlocked:(x,z,r)=>townBoundsBlocked(x,z,r)||world.colliders.some(c=>circleHitsRect(x,z,r,c)),onError:(name,error)=>console.warn('Character construction failed:',name,error)});characters.attach(player,'player',1.82);world.people.forEach(p=>characters.attach(p.g,p.g.userData.name));
...
function ensureYuri(){if(!storeClerk){storeClerk=new THREE.Group();storeClerk.userData.name='Yuri';storeClerk.visible=false;scene.add(storeClerk);characters.attach(storeClerk,'Yuri',1.88);}return storeClerk;}
...
function clearRoom(){izakayaGuests.restore();if(storeClerk)storeClerk.visible=false;roomColliders.length=0;const materials=new Set(),geos=new Set();room.traverse(o=>{for(let p=o;p&&p!==room;p=p.parent)if(p.userData.sharedAsset)return;if(o.isMesh){const list=Array.isArray(o.material)?o.material:[o.material];list.forEach(m=>m&&materials.add(m));if(![...boxCache.values()].includes(o.geometry))geos.add(o.geometry);}});materials.forEach(m=>{m.map?.dispose?.();m.dispose?.();});geos.forEach(g=>g.dispose?.());while(room.children.length)room.remove(room.children[0]);for(let i=interactables.length-1;i>=0;i--)if(interactables[i].userData?.hit?.inside)interactables.splice(i,1);}
function roomShell(s){clearRoom();town.visible=false;room.visible=true;if(s.id==='izakaya')return;if(s.id==='market'){buildStoreShell({room,box,reg,exit:leaveRoom});return;}box([13,.18,13],[0,-.08,0],0x9e957f,room,false);box([13,4.4,.24],[0,2.2,-6.45],s.color,room);box([.24,4.4,13],[-6.45,2.2,0],s.color,room);box([.24,4.4,13],[6.45,2.2,0],s.color,room);box([5,4.4,.24],[-4,2.2,6.45],s.color,room);box([5,4.4,.24],[4,2.2,6.45],s.color,room);box([3,1.4,.24],[0,3.7,6.45],s.color,room);const back=new THREE.Mesh(new THREE.PlaneGeometry(5.8,1.35),new THREE.MeshStandardMaterial({map:signTex(s.jp,s.title,s.accent),roughness:.85}));back.position.set(0,3.35,-6.28);room.add(back);const notice=new THREE.Mesh(new THREE.PlaneGeometry(3.7,1.7),new THREE.MeshStandardMaterial({map:signTex(s.title,s.line,s.accent),roughness:.85}));notice.position.set(-3.2,1.9,-6.27);room.add(notice);reg(notice,'Read wall notice',()=>activities.action('inspect',`${s.title} notice`,s.line),true);const terminal=box([2.3,2.55,.18],[.4,1.28,-5.95],0x2e3b3a,room);roomColliders.push({x:.4,z:-5.95,w:2.4,d:.32});reg(terminal,'Read the ledger',()=>activities.action('read',s.title+' ledger',s.line+'\n14 September 1988. Evening deliveries are written in blue pencil.'),true);const terminalLabel=new THREE.Mesh(new THREE.PlaneGeometry(1.95,.65),new THREE.MeshBasicMaterial({map:signTex('記録','DAILY LEDGER',s.accent)}));terminalLabel.position.set(.4,1.7,-5.83);room.add(terminalLabel);const exit=new THREE.Mesh(new THREE.PlaneGeometry(2.45,2.7),toon(0x8c684d));exit.position.set(0,1.35,6.31);exit.rotation.y=Math.PI;room.add(exit);reg(exit,'Exit to street',leaveRoom,true);}
function enterRoom(s){seated=false;if(world.isOpen&&!world.isOpen(s,minutes)){say('準備中 · Closed. Opens at '+(s.opens||'09:00')+'.',4);return;}activities.close();activities.visit(s.id);current=s;roomShell(s);addRoomProps(s);room.traverse(o=>{if(!o.isMesh||o.userData.sharedAsset)return;const b=o.geometry?.parameters;if(b?.height<.25&&o.position.y>3.8||o.position.z>6&&o.position.y>1||o.position.x>6&&o.position.y>1){o.userData.cutaway=true;o.layers.set(cam===3?0:31);}});player.position.set(0,0,4.3);yaw=0;$('#exitRoomButton').classList.remove('hidden');syncView();active=null;$('#place').textContent=s.title.toUpperCase();$('#placeSub').textContent=`${s.jp} · ${s.sub}`;$('#timeText').textContent=s.line;say(`${s.title} · Explore the miniature room. Tap objects nearby to examine them.`,3);if(cam!==3){const d=Math.max(18,13/Math.max(.42,camera.aspect));camera.position.set(8,d*.72,d);camera.lookAt(0,.6,-.4);}else{camera.position.copy(player.position);camera.position.y+=1.7;}}
function leaveRoom(){if(!current)return;inspector?.close();activities.close();resetInput();$('#directory').classList.add('hidden');$('#exitRoomButton').classList.add('hidden');izakayaGuests.restore();document.body.classList.remove('diorama');camera.fov=65;camera.updateProjectionMatrix();if(storeClerk)storeClerk.visible=false;seated=false;active=null;const s=current;current=null;syncView();room.visible=false;town.visible=true;if(s){player.position.copy(doors.get(s.id));player.position.z+=.6}$('#place').textContent='JOHANSSON TOWN';$('#placeSub').textContent='ヨハンソン町 · HARBOUR DISTRICT';$('#timeText').textContent='Shops are open.';say('Johansson Street',1.5)}
...
function residentBlocked(x,z){if(current?.id==='izakaya'&&storeClerk?.visible&&circleHitsCircle(x,z,PLAYER_RADIUS,storeClerk.position.x,storeClerk.position.z,NPC_RADIUS))return true;return world.people.some(p=>p.g.visible&&(!current||p.g.userData.inIzakaya)&&circleHitsCircle(x,z,PLAYER_RADIUS,p.g.position.x,p.g.position.z,NPC_RADIUS));}
function collides(x,z){return environmentBlocked(x,z,PLAYER_RADIUS)||residentBlocked(x,z);}
function resolveCamera(target,desired){const safe=sweepFraction(target,desired,(x,z,y)=>{const list=current?roomColliders:world.colliders;return list.some(c=>y-CAMERA_RADIUS<(c.height??3.2)&&y+CAMERA_RADIUS>(c.minY??0)&&circleHitsRect(x,z,CAMERA_RADIUS,c));},.12);const t=Math.max(0,safe-.025);camResolved.lerpVectors(target,desired,t);return camResolved;}
function updatePlayer(dt){let f=(keys.KeyW||keys.ArrowUp?1:0)-(keys.KeyS||keys.ArrowDown?1:0),s=(keys.KeyD||keys.ArrowRight?1:0)-(keys.KeyA||keys.ArrowLeft?1:0);if(moveTouch.id!==null){s+=THREE.MathUtils.clamp((moveTouch.x-moveTouch.cx)/48,-1,1);f+=THREE.MathUtils.clamp(-(moveTouch.y-moveTouch.cy)/48,-1,1);}if(seated){s=0;f=0;}move2.set(s,f);if(move2.lengthSq()>1)move2.normalize();const movementYaw=current&&cam!==3?Math.atan2(8,Math.max(18,13/Math.max(.42,camera.aspect))):yaw;fwVec.set(-Math.sin(movementYaw),0,-Math.cos(movementYaw));rtVec.set(Math.cos(movementYaw),0,-Math.sin(movementYaw));moveVec.copy(fwVec).multiplyScalar(move2.y).addScaledVector(rtVec,move2.x);if(moveVec.lengthSq()>.02){const speed=(keys.ShiftLeft||keys.ShiftRight||touchRunning||activities.state.sprintUntil>performance.now()?5.2:3)*dt,nx=player.position.x+moveVec.x*speed,nz=player.position.z+moveVec.z*speed;if(!collides(nx,player.position.z))player.position.x=nx;if(!collides(player.position.x,nz))player.position.z=nz;turnQ.setFromAxisAngle(yAxis,Math.atan2(-moveVec.x,-moveVec.z));player.quaternion.slerp(turnQ,1-Math.pow(.001,dt));}if(current&&cam!==3){player.visible=true;const aspect=Math.max(.42,camera.aspect),distance=Math.max(18,13/aspect);camDesired.set(8,distance*.72,distance);if(cameraNeedsSnap)camera.position.copy(camDesired);else camera.position.lerp(camDesired,1-Math.exp(-dt*5));cameraNeedsSnap=false;camera.lookAt(0,.6,-.4);return;}if(cam===3){cameraNeedsSnap=false;player.visible=false;camera.position.copy(player.position).add(fwVec.set(0,seated?1.15:1.7+(move2.lengthSq()>.02?Math.sin(elapsed*11)*.018:0),0));camera.rotation.order='YXZ';camera.rotation.set(pitch,yaw,0);return;}const dist=4.8,h=Math.cos(pitch)*dist;camTarget.copy(player.position).add(fwVec.set(0,1.3,0));camDesired.set(camTarget.x+Math.sin(yaw)*h,camTarget.y+1+Math.sin(-pitch)*dist,camTarget.z+Math.cos(yaw)*h);const resolved=resolveCamera(camTarget,camDesired);if(cameraNeedsSnap)camera.position.copy(resolved);else camera.position.lerp(resolved,1-Math.pow(.00005,dt));cameraNeedsSnap=false;camera.lookAt(camTarget);player.visible=camera.position.distanceTo(camTarget)>1.05;}
...
function syncView(){
 cameraNeedsSnap=true;const first=cam===3,miniature=!!current&&!first;
 document.body.classList.toggle('diorama',miniature);camera.fov=miniature?38:65;camera.updateProjectionMatrix();
 room.traverse(o=>{if(o.userData.cutaway)o.layers.set(first?0:31);});
 const mode=first?'First person':'Third person';$('#cameraButton').textContent='View · '+mode;
 $('#viewButton').textContent=first?'1st person ⇄':'3rd person ⇄';$('#viewButton').setAttribute('aria-label','Switch to '+(first?'third':'first')+' person');
 $('#viewButton').setAttribute('aria-pressed',String(first));window.__JOHANSSON_CAMERA_MODE__=first?'first':'third';
}
function toggleCamera(){if(activities?.paused||inspector?.active)return;cam=cam===3?0:3;resetInput();syncView();activities.state.cameraMode=cam===3?'first':'third';activities.save();say(cam===3?'First person · See through your own eyes.':current?'Third person · Explore the little diorama.':'Third person · Your adventure, in view.',3);}
const placeDirections=s=>s.id==='izakaya'?'Take the eastern lane from the main street and follow the red IZAKAYA sign.':s.id==='tea-house'?'Follow the residential lane north, then look for the blue-and-white curtains.':'Follow the main street to '+s.title+'.';
...
function visitPlace(s){
 ...
 if(current)leaveRoom();const p=doors.get(s.id);player.position.set(p.x,groundHeight(p.x,p.z),p.z+(s.id==='izakaya'?-1.2:.7));yaw=s.id==='izakaya'?Math.PI:s.side?-s.side*Math.PI/2:0;resetInput();toggleDir(false);navigationTarget=null;drawMap();say(s.title+' · You took a familiar shortcut.',5);return true;
}
...
function runStabilityChecks(){const failures=[];if(!townBoundsBlocked(160,0,PLAYER_RADIUS))failures.push('town edge');if(!roomBoundsBlocked(5.5,0,PLAYER_RADIUS))failures.push('room edge');if(world.colliders.length<20)failures.push('world collider coverage');if((world.quality?.streetInteractions||0)<8)failures.push('street interaction coverage');for(const [id,p] of doors)if(environmentBlocked(p.x,p.z,PLAYER_RADIUS)||environmentBlocked(p.x,p.z+.7,PLAYER_RADIUS))failures.push(`door spawn ${id}`);window.__JOHANSSON_STABILITY__={ok:failures.length===0,failures,colliders:world.colliders.length,characterCount:world.people.length+1,streetInteractions:world.quality?.streetInteractions||0,renderDpr:renderer.getPixelRatio(),toneMapping:'AgX',shadows};if(failures.length)console.error('Johansson Town stability checks failed',failures);else console.info('Johansson Town stability checks passed',window.__JOHANSSON_STABILITY__)}
...
function setConversation(name){
 ...
 if(name){
  ...
  if(speaker&&!current){const target=speaker.getWorldPosition(new THREE.Vector3());target.y+=1.25;camera.lookAt(target);}
...
function simulate(frameDt){
  ...
  while(accumulator>=SIM_STEP){
    ...
    if(current?.id==='izakaya')izakayaGuests.sync(minutes);if(!current){world.update(dt,elapsed,daylight(minutes),minutes);castAI?.update(dt,minutes,weather);if(!activities.state.quickTravelNotified&&travelProgress(activities.state).unlocked)activities.save();world.beats?.update(dt,elapsed,minutes);}
    accumulator-=SIM_STEP;
  }
}
let controlsMovingUntil=0;
function updateContextControls(){
 const blocked=activities.paused||inspector?.active||!$('#directory').classList.contains('hidden')||!$('#qte').classList.contains('hidden');
...
function loop(){requestAnimationFrame(loop);updateContextControls();const frameDt=Math.min(clock.getDelta(),MAX_FRAME_DT);if(started&&!document.hidden){const paused=inspector?.active||activities.paused||!$('#directory').classList.contains('hidden');if(!paused){const before=player.position.clone();simulate(frameDt);if(player.position.distanceTo(before)>.01&&(stepTick+=frameDt)>.42){activities.footstep(current?'wood':routeAt(player.position.x,player.position.z)?.surface||'stone');stepTick=0;}interaction();}else{accumulator=0;resetInput();$('#prompt').classList.remove('on');}townAudio.update({player:player.position,yaw,minutes,rain:weather,inside:!!current,station:activities.state.radioStation||0,paused});$('#clock').textContent=fmt(minutes);setTime();hands?.update(paused?0:frameDt);if(!inspector?.active){characters?.update(frameDt);castAI?.pose(frameDt);}if((mapTick+=frameDt)>.15){drawMap();mapTick=0;}if(subtitleTimer>0&&(subtitleTimer-=frameDt)<=0)$('#subtitle').classList.remove('on');if(inspector?.active)inspector.render(frameDt);else renderer.render(scene,camera)}}renderer.render(scene,camera);loop();
