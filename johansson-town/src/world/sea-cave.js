...
export const SEA_CAVE=Object.freeze({centerX:-15,frontZ:36,scale:.32,angle:Math.PI,groundY:-.65});
export const SEA_CAVE_BARRIER=Object.freeze({x:0,z:34.4,w:15.6,d:.35});
...
export function buildSeaCave(world,options={}){
 ...
 for(const [x,y,z,sx,sy,sz] of [[-20,2,44,11,5,7],[-8,2.6,46,10,6,8],[3,1.5,46,6,3.5,6]]){const rock=new THREE.Mesh(rockGeo,rockMat);rock.position.set(x,y,z);rock.scale.set(sx,sy,sz);fallback.add(rock);}
 ...
 const marker=new THREE.Object3D();marker.position.set(0,1.2,32);group.add(marker);
