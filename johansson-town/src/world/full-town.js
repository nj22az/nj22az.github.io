...
import {FULL_TOWN,FULL_PATHS,sourceHeight,extensionHeight,fullContains} from './full-town-state.js';
...
export function preloadFullTown(){return pending??=(async()=>{
 ...
 try{const [asset,data]=await Promise.all(['overworld.glb','navigation.json'].map(file=>fetch(assetURL('models/full-town/'+file),{signal:controller.signal})));if(!asset.ok||!data.ok)throw Error('Town assets unavailable');const navigation=await data.json();const loaded=await new GLTFLoader().parseAsync(await asset.arrayBuffer(),'');
  ...
  source=loaded.scene;FULL_TOWN.grid=navigation.grid;FULL_TOWN.colliders=navigation.colliders;FULL_TOWN.active=true;return true;
...
export function buildFullTown(options){
 ...
 const model=source.clone(true);model.userData.sharedAsset=true;model.name='Original canal, bridge, buildings and streets';model.traverse(o=>{if(!o.isMesh)return;o.castShadow=!!options.shadows;o.receiveShadow=true;for(const map of [o.material.map,o.material.normalMap,o.material.roughnessMap])if(map)map.anisotropy=Math.min(options.maxAnisotropy||1,4);});group.add(model);
 const colliders=FULL_TOWN.colliders.map(c=>({...c})),world={group,colliders,people:[],homes:new Map(),harbourShops:[],plantSites:[],quality:{completeSuppliedOverworld:true,streetInteractions:18,localRuntimeAssets:true,sourceTriangles:96308},isOpen(site,minutes){const m=((minutes%1440)+1440)%1440;return site.id==='izakaya'?izakayaOpen(m):site.id==='home'||site.id==='bus-hut'||m>=540&&m<(site.id==='market'?1200:site.id==='ramen'?1260:1140);},updateHours(){},updateHomes(){},setRain(value){rain.visible=value;},update(dt,time){if(rain.visible){rain.rotation.y=time*.01;rain.position.y=-(time*7)%6;}}};
 ...
 for(const [id,x,z,nx,nz] of LOCATIONS){
  ...
  anchor(p,'Enter '+site.title,()=>options.enter(site));sign(site.jp,x+nx*.18,2.55,z+nz*.18,Math.atan2(nx,nz));
 }
 ...
 // Residents occupy the original buildings, with separately addressed upstairs homes.
 ...
 RESIDENTS.forEach((profile,i)=>{
  ...
  profile.work=work;profile.evening=nearest(i%2?-5:4,i%3===0?-1:3);profile.home=home;profile.homeAddress=(i%2?'2':'1')+'F · '+FULL_TOWN.sites.get(homeSite).title;
 ...
 FULL_TOWN.patrol=[[-5,-1],[-5,-5],[-11,-6],[ -11,1],[4,0],[10,1],[17,0],[4,0]].map(p=>nearest(...p));
 ...
 // An adjoining quay and the already supplied park connect at the model boundary.
 const vertices=[],indices=[],step=.25;
 for(let z=-35;z<2;z+=step)for(let x=-10;x<30;x+=step){
  if(sourceHeight(x+step/2,z+step/2)!==null||parkHeight(x+step/2,z+step/2)!==null||extensionHeight(x+step/2,z+step/2,parkHeight)===null)continue;
  const corners=[[x,z],[x,z+step],[x+step,z],[x+step,z+step]],base=vertices.length/3;
  for(const p of corners)vertices.push(p[0],groundHeight(...p)-.012,p[1]);indices.push(base,base+1,base+2,base+2,base+1,base+3);
 }
 ...
 world.park.seat={...world.park.seat,yaw:2.15,pitch:0};world.park.bench.userData.seat=world.park.seat;
 anchor([0,-31],'Fish from the harbour pier',()=>options.onAction('fishing'));
 ...
 // Visible activity props sit along the quay, leaving the central walking lane clear.
 for(const [i,kind,title] of [[0,'phone','Harbour payphone'],[1,'vending','Buy a cold drink'],[2,'arcade','Star Port arcade'],[3,'radio','Neighbourhood radio']]){
  const x=3+i*3,z=-20.9,g=new THREE.Group();g.position.set(x,0,z);group.add(g);
  if(kind==='vending')g.add(createVendingMachine({shadows:options.shadows}));
  else{box([.8,kind==='radio'?1:1.6,.6],[0,kind==='radio'?.5:.8,0],kind==='phone'?0x536b58:0x655547,g);box([.58,.42,.04],[0,1.15,.32],0x304d4a,g);}
  colliders.push({x,z,w:1.1,d:.65});anchor([x,-20],title,()=>options.onAction(kind,title));
 }
 const bench=box([1.8,.15,.65],[15,.55,-20.9],0x795d43);box([1.8,.65,.12],[15,.9,-21.18],0x795d43);for(const x of [14.3,15.7])box([.12,.55,.5],[x,.275,-20.9],0x4d453a);
 colliders.push({x:15,z:-20.9,w:1.8,d:.65});const seat=anchor([15,-20],'Sit beside the harbour',()=>options.onAction('seat','Harbour bench'));seat.userData.seat={position:[15,0,-20.9],stand:[15,0,-20],eyeY:1.3,yaw:0,pitch:0};
 for(const [title,ideal] of [['Bus timetable',[-18,0]],['Canal crossing',[0,0]],['Harbour notices',[-5,-19]]])anchor(nearest(...ideal),title,()=>options.onAction('read',title,'Johansson Town · Canal quarter and harbour · 14 September 1988'));
