...
export function buildFullTown(options){
 ...
 const colliders=FULL_TOWN.colliders.map(c=>({...c})),world={group,colliders,people:[],homes:new Map(),harbourShops:[],plantSites:[],quality:{completeSuppliedOverworld:true,streetInteractions:18,localRuntimeAssets:true,sourceTriangles:96308,citySections:CITY_SECTIONS.length,removedStreetColliders:clearance.removedColliders,removedStreetTriangles:clearance.removedTriangles},isOpen(site,minutes){const m=((minutes%1440)+1440)%1440;return site.id==='izakaya'?izakayaOpen(m):site.id==='home'||site.id==='bus-hut'||m>=540&&m<(site.id==='market'?1200:site.id==='ramen'?1260:1140);},updateHours(){},updateHomes(){},setRain(value){rain.visible=value;},update(dt,time){if(rain.visible){rain.rotation.y=time*.01;rain.position.y=-(time*7)%6;}}};
 ...
 for(const [i,kind,title] of [[0,'phone','Harbour payphone'],[1,'vending','Buy a cold drink'],[2,'arcade','Star Port arcade'],[3,'radio','Neighbourhood radio']]){
  const x=3+i*3,z=-21.75,g=new THREE.Group();g.position.set(x,0,z);group.add(g);
 ...
 const bench=box([1.8,.15,.65],[15,.55,-21.75],0x795d43);box([1.8,.65,.12],[15,.9,-22.03],0x795d43);for(const x of [14.3,15.7])box([.12,.55,.5],[x,.275,-21.75],0x4d453a);
 colliders.push({x:15,z:-21.75,w:1.8,d:.65});const seat=anchor([15,-20],'Sit beside the harbour',()=>options.onAction('seat','Harbour bench'));seat.userData.seat={position:[15,0,-21.75],stand:[15,0,-20],eyeY:1.3,yaw:0,pitch:0};
 for(const [title,ideal] of [['East quarter',[39,0]],['Bus timetable',[-18,0]],['Canal crossing',[0,0]],['Harbour notices',[-5,-19]]])anchor(nearest(...ideal),title,()=>options.onAction('read',title,'Johansson Town · Canal quarter and harbour · 14 September 1988'));
