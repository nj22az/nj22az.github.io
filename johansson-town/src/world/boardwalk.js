...
export function buildBoardwalk(parent,options={}){
 ...
 for(let i=0;i<count;i++){dummy.position.set(BOARDWALK.x,.001,BOARDWALK.minZ+i*.36);dummy.scale.set(BOARDWALK.width-.12,.003,.013);dummy.updateMatrix();joints.setMatrixAt(i,dummy.matrix);}
 ...
 for(const [i,side] of [-1,1].entries()){dummy.position.set(BOARDWALK.x+side*(BOARDWALK.width/2-.07),-.014,(BOARDWALK.minZ+BOARDWALK.maxZ)/2);dummy.scale.set(1,1,1);dummy.updateMatrix();edges.setMatrixAt(i,dummy.matrix);}
