...
export function buildDistricts(world,options){
  ...
  function building({id,x,z,w=8,d=7,h=6,colour=0xbeb394,roof=0,angle=0,jp,title,frontZ=z+d/2}){
    ...
    if(roof===2){box([w+.65,.22,d+.65],[x,h+.15,z],'plaster',0x8e9487);box([1.7,1.5,1.8],[x+1.6,h+1,z],'plaster',0xa9a291);}
    else {const shape=new THREE.Shape();shape.moveTo(-w/2-.55,0);shape.lineTo(0,roof===1?.55:1.4);shape.lineTo(w/2+.55,0);shape.lineTo(w/2+.55,-.18);shape.lineTo(0,(roof===1?.55:1.4)-.18);shape.lineTo(-w/2-.55,-.18);shape.closePath();const geo=new THREE.ExtrudeGeometry(shape,{depth:d+1,bevelEnabled:false});const mesh=new THREE.Mesh(geo,library.material('roof',roof===1?0x8c7c69:0x656f6a));mesh.position.set(x,h,z-d/2-.5);mesh.castShadow=options.shadows;group.add(mesh);}
    for(const side of [-1,1]){
      const sx=x+side*w*.29;box([1.7,1.45,.1],[sx,h-1.3,frontZ+.17],'timber',0x45594f);box([.06,1.52,.13],[sx,h-1.3,frontZ+.22],'plaster',0xb1b4a7);
    ...
    for(const side of [-1,1])box([.06,2.6,.12],[x+side*1.05,1.3,frontZ+.2],'plaster',0xb5b6aa);
    const door=new THREE.Mesh(new THREE.BoxGeometry(2.1,2.6,.13),library.material('timber',0x747e6b));door.position.set(x,4,frontZ+.15);group.add(door);shutters.push({mesh:door,id});
    ...
    box([1.1,.65,.65],[x-w*.37,3.68,frontZ+.42],'plaster',0xb8b4a1);for(let n=0;n<4;n++)box([.85,.04,.04],[x-w*.37,3.49+n*.11,frontZ+.77],'roof',0x605f53);
    box([.35,.55,.22],[x+w*.43,1.8,frontZ+.22],'plaster',0x7e8176);
    const pot=new THREE.Mesh(new THREE.CylinderGeometry(.26,.2,.45,9),library.material('plaster',0x9c7354));pot.position.set(x-w*.42,.25,frontZ+.65);group.add(pot);const plant=new THREE.Mesh(new THREE.IcosahedronGeometry(.34,1),new THREE.MeshStandardMaterial({color:0x566e42,roughness:1}));plant.position.copy(pot.position).y=.71;group.add(plant);
  ...
  for(const x of [-84,-72])box([.6,2,74],[x,-.7,-24],'plaster',0x808f83);
  for(const [x,z] of [[-75,-40],[-80,-22]]){const bird=new THREE.Group();bird.position.set(x,.18,z);const body=new THREE.Mesh(new THREE.SphereGeometry(.18,8,6),new THREE.MeshStandardMaterial({color:0xb8bcb0,roughness:1}));body.scale.set(1,1.6,1);bird.add(body);for(const dx of [-.08,.08])box([.025,.45,.025],[x+dx,.1,z],'timber',0x4e4d40);group.add(bird);verb([-69,1,-37],'Watch the heron','inspect','Grey heron','It waits for a fish to come to it. An admirable working arrangement.');}
  sign('小学校','GATE CLOSES AT 16:00',[67,2.2,45],3,.8);box([8,1.6,.12],[67,.8,44.5],'timber',0x687566);colliders.push({x:67,z:44.5,w:8,d:.12,height:1.6});verb([67,1,46],'Look through school gate','read','School gate','The last baseball practice has finished. Indoor shoes stand in neat rows beyond the locked gate.');
  // Shrine stair/approach culminates in a real raised landing.
  box([8,.4,8],[72,5.8,117],'plaster',0xa2a18c);for(const x of [69,75])box([.25,4,.25],[x,8,115],'timber',0x9b4833);box([8,.28,.45],[72,10,115],'timber',0x943d2a);verb([72,7.3,117],'Visit hillside shrine','shrine','Hillside shrine','The bay lies below the roofs.');
  for(const [x,z] of [[69,119],[72,119],[75,119]]){box([.65,1.4,.65],[x,6.7,z],'plaster',0x7f877a);colliders.push({x,z,w:.65,d:.65,minY:6,height:7.4});verb([x,7,z],'Read memorial stone','read','Family memorial','Fresh water, incense and a small bunch of autumn flowers.');}
