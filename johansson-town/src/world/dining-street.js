...
import {unfoldDiningStreet} from './unfold-dining.js';
...
export function preloadDiningStreet(){
 ...
 pending=(async()=>{const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),25000);
  try{const response=await fetch(assetURL('models/dining-street/night-lane.glb'),{signal:controller.signal});if(!response.ok)throw Error(response.status);
   const loaded=await new GLTFLoader().parseAsync(await response.arrayBuffer(),'');if(!loaded.scene.children.length)throw Error('Empty dining street');closeDiningBacks(loaded.scene);source=unfoldDiningStreet(loaded.scene);return true;
...
export function buildDiningStreet(world,options={}){
 const group=new THREE.Group();group.name='Main Street alley shopfronts';group.position.y=NIGHT_LANE.y;world.group.add(group);
