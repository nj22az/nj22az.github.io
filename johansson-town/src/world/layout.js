...
export const ROUTES = [
 ...
 {id:'east-alley',width:3.8,surface:'stone',points:[[0,18],[38,18],[38,50],[0,50]]},
 {id:'west-alley',width:3.6,surface:'stone',points:[[0,50],[-40,50],[-40,-51],[-27,-51]]},
 {id:'residential',width:3.4,surface:'stone',points:[[38,30],[46,30],[46,42],[38,42]]},
 ...
 {id:'izakaya-door',width:4,surface:'asphalt',points:[[DINING.izakayaX,18],DINING.izakayaDoor]},
 {id:'ramen-door',width:4,surface:'stone',points:[[DINING.ramenDoor[0],18],DINING.ramenDoor]},
 {id:'crystal-door',width:2.4,surface:'stone',points:[[DINING.crystalDoor[0],18],DINING.crystalDoor]},
 {id:'bus-door',width:4,surface:'stone',points:[[-26,50],[-26,46]]},
 {id:'school-route',width:3.4,surface:'stone',points:[[46,30],[46,36],[44,36]]},
 {id:'park-walk',width:4,surface:'stone',points:[[18,-58],[27,-58],[27,-52]]},
 {id:'park-approach',width:3,surface:'stone',points:[[16,-38],[21,-38]]},
 {id:'home-lane',width:3.4,surface:'stone',points:[[RESIDENTIAL.laneX,-16],[RESIDENTIAL.laneX,7]]},
 ...
 {id:'east-service',width:3,surface:'stone',points:[[18,-50],[18,-46],[16,-46],[16,12],[20.8,12],[20.8,18]]},
 {id:'east-quay',width:3,surface:'stone',points:[[18,-50],[18,-58]]},
 {id:'east-market-cut',width:3,surface:'stone',points:[[0,-28],[16,-28]]},
 {id:'east-workshop-cut',width:3,surface:'stone',points:[[0,-4],[16,-4]]},
...
export function routeAt(x,z,r=0){if(FULL_TOWN.active)return fullContains(x,z,r,parkHeight)?{id:"supplied-town",surface:"stone"}:null;if(inDiningLane(x,z))return {id:'dining-lane',surface:'asphalt'};if(inResidential(x,z))return residentialContains(x,z,r)?{id:'willow-alley',surface:'stone'}:null;if(Math.abs(x-PARK.x)<=14-r&&Math.abs(z-PARK.z)<=14-r)return PARK;const route=LANDINGS.find(p=>Math.abs(x-p.x)<=p.w/2-r&&Math.abs(z-p.z)<=p.d/2-r)||ROUTES.find(route=>route.points.slice(1).some((b,i)=>nearestOnSegment(x,z,route.points[i],b).d<=route.width/2-r));return route?.id==='shotengai'&&z>=BOARDWALK.minZ&&z<=BOARDWALK.maxZ?boardwalkRoute:route;}
export function groundHeight(x,z){if(FULL_TOWN.active)return fullHeight(x,z,parkHeight);if(inDiningLane(x,z))return NIGHT_LANE.y;const rh=residentialHeight(x,z);if(rh!==null)return rh;if(x>=17.5&&x<21&&Math.abs(z+38)<=1.5)return (x-17.5)/3.5*parkHeight(21,z);const ph=parkHeight(x,z);if(ph!==null)return ph;if(Math.abs(x-27)<=2.1&&z>=-58&&z< -52)return (z+58)/6*parkHeight(27,-52);if(Math.abs(x-OUTER_PIER.x)<=OUTER_PIER.width/2&&Math.abs(z-OUTER_PIER.z)<=OUTER_PIER.length/2)return OUTER_PIER.height;const landing=LANDINGS.find(p=>Math.abs(x-p.x)<=p.w/2&&Math.abs(z-p.z)<=p.d/2);if(landing)return landing.height;const slope=ROUTES.find(r=>r.id==='shrine-slope');let best=null,done=0,total=0;for(let i=1;i<slope.points.length;i++)total+=Math.hypot(slope.points[i][0]-slope.points[i-1][0],slope.points[i][1]-slope.points[i-1][1]);for(let i=1;i<slope.points.length;i++){const a=slope.points[i-1],b=slope.points[i],n=nearestOnSegment(x,z,a,b),len=Math.hypot(b[0]-a[0],b[1]-a[1]);if(!best||n.d<best.d)best={...n,height:(done+n.t*len)/total*6};done+=len;}return best.d<4?best.height:0;}
