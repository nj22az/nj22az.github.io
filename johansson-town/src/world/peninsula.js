...
import {COASTLINE,pointInCoastline,peninsulaLandContains} from './coastline.js';
export {COASTLINE,pointInCoastline,peninsulaLandContains};
...
export function buildPeninsula(parent){
 ...
 const ground=new THREE.Mesh(new THREE.ShapeGeometry(shape),new THREE.MeshStandardMaterial({color:0x8b9279,roughness:1}));
 ground.rotation.x=-Math.PI/2;ground.position.y=-.02;ground.name='Peninsula land';parent.add(ground);
 const positions=[],indices=[];
 for(let i=0;i<COASTLINE.length;i++){
  const a=COASTLINE[i],b=COASTLINE[(i+1)%COASTLINE.length],n=positions.length/3;
  positions.push(a[0],-.02,a[1],a[0],-1.5,a[1],b[0],-.02,b[1],b[0],-1.5,b[1]);indices.push(n,n+1,n+2,n+2,n+1,n+3);
 }
 ...
 rocks.forEach((r,i)=>{dummy.position.set(r.x,.04,r.z);dummy.scale.set(r.size*1.6,r.size*.7,r.size);dummy.rotation.set(.2,i*1.7,.15);dummy.updateMatrix();shore.setMatrixAt(i,dummy.matrix);});
 shore.name='Rocky peninsula shore';parent.add(shore);
 return ground;
}
