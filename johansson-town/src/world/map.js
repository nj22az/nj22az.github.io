import {CITY_SECTIONS,FULL_TOWN,FULL_PATHS} from './full-town-state.js';
...
export function drawTownMap(ctx,w,h,{sites=[],people=[],player={x:0,z:0},yaw=0,visited=[],target=null}={}) {
  ...
  if(FULL_TOWN.active){
    ...
    for(const section of CITY_SECTIONS){
      ctx.fillStyle='#a1b9b3';ctx.fillRect(px(section.x-3.4),pz(section.z+16),6.8*scale,30*scale);
