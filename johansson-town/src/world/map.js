...
export function drawTownMap(ctx,w,h,{sites=[],people=[],player={x:0,z:0},yaw=0,visited=[],target=null}={}) {
  ...
  if(FULL_TOWN.active){const g=FULL_TOWN.grid;ctx.fillRect(px(-3.4),pz(16),6.8*scale,30*scale);ctx.fillStyle='#a8997a';for(let iz=0;iz<g.nz;iz++)for(let ix=0;ix<g.nx;ix++)if(g.heights[iz*g.nx+ix]!==null)ctx.fillRect(px(g.minX+ix*g.step),pz(g.minZ+iz*g.step),Math.max(1,g.step*scale),Math.max(1,g.step*scale));ctx.fillStyle='#6c6656';for(const c of FULL_TOWN.colliders)ctx.fillRect(px(c.x-c.w/2),pz(c.z+c.d/2),Math.max(1,c.w*scale),Math.max(1,c.d*scale));}
