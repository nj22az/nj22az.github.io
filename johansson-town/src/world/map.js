...
import {TUNNEL,tunnelEndZ} from './coyote-tunnel.js';
...
export function drawTownMap(ctx,w,h,{sites=[],landmarks=[],people=[],player={x:0,z:0},yaw=0,visited=[],target=null}={}) {
  ...
  if(peninsulaActive()){ctx.fillStyle='#6d6a60';ctx.fillRect(px(TUNNEL.x-TUNNEL.width/2),pz(TUNNEL.z+TUNNEL.depth),Math.max(1,TUNNEL.width*scale),Math.max(2,TUNNEL.depth*scale));ctx.strokeStyle='#766c50';ctx.lineWidth=Math.max(2,BOARDWALK.width*scale);ctx.beginPath();ctx.moveTo(px(TUNNEL.x),pz(FOREST_EDGE.roadEndZ));ctx.lineTo(px(TUNNEL.x),pz(tunnelEndZ()));ctx.stroke();if(w>=300){ctx.fillStyle='#304d40';ctx.font='bold 10px sans-serif';ctx.fillText('TUNNEL',px(TUNNEL.x-TUNNEL.width/2+.5),pz(TUNNEL.z+TUNNEL.depth*.6));}}
