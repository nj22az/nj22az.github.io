...
export function drawTownMap(ctx,w,h,{sites=[],people=[],player={x:0,z:0},yaw=0,visited=[],target=null}={}) {
  ...
  for(const route of (FULL_TOWN.active?FULL_PATHS:ROUTES)){
    ctx.strokeStyle=route.surface==='wood'||/quay|port-walk|harbour-apron|city-link/.test(route.id)?'#a28459':'#766c50';
  ...
  for(const site of sites){ctx.fillStyle=visited.includes(site.id)?'#a65739':'#4d6156';const x=site.x??site.side*11.8;ctx.fillRect(px(x)-2.3,pz(site.z)-3,4.6,6);
