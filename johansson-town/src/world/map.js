import {FULL_TOWN,FULL_PATHS} from './full-town-state.js';
...
export function drawTownMap(ctx,w,h,{sites=[],people=[],player={x:0,z:0},yaw=0,visited=[],target=null}={}) {
  const b=FULL_TOWN.active?FULL_TOWN.bounds:MAP_BOUNDS,pad=10,scale=Math.min((w-pad*2)/(b.maxX-b.minX),(h-pad*2)/(b.maxZ-b.minZ));
  const px=x=>pad+(x-b.minX)*scale,pz=z=>h-pad-(z-b.minZ)*scale;
  ctx.fillStyle='#eadfbe';ctx.fillRect(0,0,w,h);
  ctx.fillStyle='#a1b9b3';ctx.fillRect(0,pz(FULL_TOWN.active?-14:-62),w,h-pz(FULL_TOWN.active?-14:-62));
  if(FULL_TOWN.active){const g=FULL_TOWN.grid;ctx.fillRect(px(-3.4),pz(16),6.8*scale,30*scale);ctx.fillStyle='#a8997a';for(let iz=0;iz<g.nz;iz++){let start=-1;for(let ix=0;ix<=g.nx;ix++){const land=ix<g.nx&&g.heights[iz*g.nx+ix]!==null;if(land&&start<0)start=ix;if(!land&&start>=0){ctx.fillRect(px(g.minX+start*g.step),pz(g.minZ+iz*g.step),Math.max(1,(ix-start)*g.step*scale),Math.max(1,g.step*scale));start=-1;}}}ctx.fillStyle='#6c6656';for(const c of FULL_TOWN.colliders)ctx.fillRect(px(c.x-c.w/2),pz(c.z+c.d/2),Math.max(1,c.w*scale),Math.max(1,c.d*scale));}
  ...
  for(const route of (FULL_TOWN.active?FULL_PATHS:ROUTES)){ctx.lineWidth=Math.max(2,route.width*scale);ctx.beginPath();route.points.forEach(([x,z],i)=>i?ctx.lineTo(px(x),pz(z)):ctx.moveTo(px(x),pz(z)));ctx.stroke();}
