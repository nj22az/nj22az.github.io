...
p=argparse.ArgumentParser();p.add_argument('--root',required=True);p.add_argument('--poses',required=True);a=p.parse_args(sys.argv[sys.argv.index('--')+1:]);root=Path(a.root).resolve()
...
s=bpy.context.scene;s.render.engine='CYCLES';s.cycles.samples=24;s.render.resolution_x=600;s.render.resolution_y=720;s.render.resolution_percentage=100
...
out=root/'art/characters/protagonist';out.mkdir(parents=True,exist_ok=True)
