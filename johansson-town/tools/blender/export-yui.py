"""Finish the Blender-authored Yui and export a self-contained animated GLB.
Run after build-resident.py, with --root pointing to the game directory.
"""
import bpy,sys,argparse,math,json,os
from pathlib import Path
from mathutils import Vector,Quaternion,Matrix
p=argparse.ArgumentParser();p.add_argument('--root',required=True);args=p.parse_args(sys.argv[sys.argv.index('--')+1:]);root=Path(args.root);out=root/'art/characters/yui'
bpy.ops.wm.open_mainfile(filepath=str(out/'yui-review.blend'))
rig=bpy.data.objects['Yui.Rig'];meshes=[o for o in bpy.data.objects if o.type=='MESH' and o.name.startswith('Yui.')]
# Re-encode scaled texture pixels before packing so the .blend and GLB share the same sizes.
texture_dir=Path('/tmp/town-yui-textures');texture_dir.mkdir(exist_ok=True)
for image in list(bpy.data.images):
 if image.source not in ['FILE','GENERATED'] or image.name=='Render Result':continue
 ...
 opaque=not any(x in image.name for x in ['long01','eyebrow','normal'])
...
# Put authored accessory vertices into rig space before skinning, so their offset
# cannot rotate around the object origin when the chest moves.
for o in meshes:
 if any(t in o.name for t in ['Collar','Glasses','Beret','Ribbon']):
  bpy.ops.object.select_all(action='DESELECT');o.select_set(True);bpy.context.view_layer.objects.active=o
  bpy.ops.object.transform_apply(location=True,rotation=True,scale=True)
  if not any(m.type=='ARMATURE' for m in o.modifiers):
   mod=o.modifiers.new('Armature','ARMATURE');mod.object=rig
# Standardise the hierarchy and keep the actual standing height at Yui's 1.62 metres.
for o in meshes:
...
bpy.context.view_layer.update();bounds=[o.matrix_world@Vector(c) for o in meshes for c in o.bound_box];height=max(v.z for v in bounds)-min(v.z for v in bounds);rig.scale*=1.62/height
shoes=next(o for o in meshes if 'shoes03' in o.name)
...
def pose(phase=0,amount=0,wave=0):
 ...
 for side,sign in [('l',1),('r',-1)]:
  ...
  swing=-step*.45;aim('upperarm_'+side,(sign*.23,math.sin(swing)-.025,-1))
  aim('lowerarm_'+side,(sign*.12,math.sin(swing)-.13,-1));aim('hand_'+side,(sign*.025,math.sin(swing)-.12,-1))
 if wave:
...
for o in meshes:
 ratio=.55 if o.name=='Yui.Body' else .35 if 'elegantsuit' in o.name else .40 if 'long01' in o.name else .55 if 'shoes' in o.name else 1
 if ratio<1:
...
scene.render.resolution_x=900;scene.render.resolution_y=1100;scene.render.filepath=str(out/'yui-full.png')
bpy.ops.wm.save_as_mainfile(filepath=str(out/'yui.blend'),compress=True)
for o in meshes:
 bpy.ops.object.select_all(action='DESELECT');o.select_set(True);bpy.context.view_layer.objects.active=o
...
model=root/'assets/characters/realistic/yui.glb';model.parent.mkdir(parents=True,exist_ok=True)
...
head=rig.matrix_world@rig.pose.bones['head'].head;target=head+Vector((0,-.03,.06));cam.location=target+Vector((.28,-2.7,.07));cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=.56;scene.render.resolution_x=900;scene.render.resolution_y=900;scene.render.filepath=str(out/'yui-face.png');bpy.ops.render.render(write_still=True)
rig.animation_data.action=bpy.data.actions['Walk'];scene.frame_set(7);cam.location=(2,-5,1.4);cam.rotation_euler=(Vector((0,0,.91))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=2.04;scene.render.resolution_y=1100;scene.render.filepath=str(out/'yui-walk.png');bpy.ops.render.render(write_still=True)
