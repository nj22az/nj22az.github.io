"""Fit the owner-supplied bedroom interior for Yuri's room.

Requires NumPy and Pillow. Usage:
  python tools/pack-yuri-bedroom.py /path/to/bedroom.glb
"""
import hashlib
import io
import json
...
SOURCE_SHA = '38aa3b54b2e541bd59458452bd8a31cdcfb328c99062e67dc70141e98143ca29'
...
def pack(source):
    ...
    if hashlib.sha256(original).hexdigest() != SOURCE_SHA:
        raise ValueError('bedroom: input differs from the supplied model')
    ...
    def collect(index, parent):
        ...
        if 'mesh' in node:
            for primitive in src['meshes'][node['mesh']]['primitives']:
                ...
                if 'NORMAL' in attrs:
                    normals = attrs['NORMAL'] @ np.linalg.inv(world[:3, :3]).T
    ...
    for attrs, _, _, _ in baked:
        shifted = attrs['POSITION'] - np.array([cx, float(lo[1]), cz], dtype=np.float32)
    ...
    result = {'asset': {**src.get('asset', {}), 'generator': 'Johansson Town bedroom packer'},
    ...
    for group in groups.values():
        ...
        result['nodes'].append({'name': 'bedroom:'+group['material']['name'], 'mesh': len(result['meshes'])-1})
    ...
    manifest = {
        ...
        'changes': [
            ...
            'Scaled a 1.33 m dollhouse interior to a 2.72 m ceiling',
            'Rotated the south doorway to +Z to match other supplied rooms',
