...
import * as T from '../vendor/three.module.js';import {GLTFLoader} from '../vendor/GLTFLoader.js';import {prepareYuriAnimations} from '../src/people/yuri-animation.js';
globalThis.self=globalThis;globalThis.createImageBitmap=async()=>({width:1024,height:1024,close(){}});
const bytes=await readFile(new URL('../assets/characters/realistic/yuri-playful.glb',import.meta.url));const g=await new GLTFLoader().parseAsync(bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength),'');g.animations=prepareYuriAnimations(g);const mixer=new T.AnimationMixer(g.scene);const frames=[];
...
await writeFile(process.argv[2]||'yuri-geometry.json',JSON.stringify(frames));
