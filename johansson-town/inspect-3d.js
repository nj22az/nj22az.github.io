...
export function createInspector({scene,camera,renderer,canvas,onInspect,onReturn=()=>{},onLink,onContact,resetInput}){
  ...
  const pageBack=button('← Page',()=>held?.userData.reader.next(-1));const pageNext=button('Page →',()=>held?.userData.reader.next(1));button('FLIP / OPEN',()=>held?.userData.reader.flip());button('RESET',reset);
  ...
  return {open,close,control(input,dt){if(!held)return;held.rotation.y+=input.x*dt*2;held.rotation.x+=input.y*dt*2;zoom=THREE.MathUtils.clamp(zoom+input.zoom*dt*1.2,.8,3.5);if(input.reset)reset();},get active(){return !!held;},render(dt){if(!held)return;overlayCamera.aspect=camera.aspect;overlayCamera.updateProjectionMatrix();held.position.set(pan.x,pan.y,-zoom);light.position.set(0,1,-zoom+1);held.userData.reader.update(dt);title.textContent=item.title+(item.kind==='printed-model'?' · Form 3D':' · '+held.userData.reader.page+'/'+item.pages.length)+' · Drag rotate · Right-drag pan · IJKL orbit · E / Shift+Q zoom · Q close';renderer.render(scene,overlayCamera);if(item.kind==='printed-model'){const autoClear=renderer.autoClear;renderer.autoClear=false;renderer.clearDepth?.();renderer.render(modelScene,overlayCamera);renderer.autoClear=autoClear;}}};
}
