...
export function createTown({scene,sites,mobile,register,enter,onAction}) {
  const group=new THREE.Group();scene.add(group);
  const materials=new Map(),geometries=new Map(),batches=new Map(),colliders=[],lamps=[],people=[],water=[];
  const loader=new THREE.TextureLoader();
  function texture(file,repeat){const t=loader.load(new URL(`./assets/${file}`,import.meta.url).href,undefined,undefined,()=>{});t.colorSpace=THREE.SRGBColorSpace;t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(...repeat);t.anisotropy=mobile?2:4;return t;}
  ...
  function material(color,map,glow=0){const key=`${color}/${map||''}/${glow}`;if(!materials.has(key))materials.set(key,new THREE.MeshStandardMaterial({color,map:maps[map]||null,roughness:map==='road'?.87:.92,emissive:glow?color:0,emissiveIntensity:glow}));return materials.get(key);}
  ...
  const cyl=(r,h,p,c=0x444b4b)=>shape('cylinder',[r,r,h,8],p,c);
  function beam(a,b,r=.025,c=0x303334){const d=new THREE.Vector3().subVectors(new THREE.Vector3(...b),new THREE.Vector3(...a));const q=new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0,1,0),d.clone().normalize());const e=new THREE.Euler().setFromQuaternion(q);shape('cylinder',[r,r,d.length(),6],a.map((v,i)=>(v+b[i])/2),c,[e.x,e.y,e.z]);}
  function label(text,sub,p,width,height,angle=0,bg='#e9dcc1',fg='#283d3e',glow=false){const canvas=document.createElement('canvas');canvas.width=768;canvas.height=256;const ctx=canvas.getContext('2d');ctx.fillStyle=bg;ctx.fillRect(0,0,768,256);ctx.strokeStyle=fg;ctx.lineWidth=8;ctx.strokeRect(12,12,744,232);ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle=fg;ctx.font='700 96px "Yu Gothic",system-ui';ctx.fillText(text,384,106,716);ctx.font='600 30px system-ui';ctx.fillText(sub,384,201,700);const tex=new THREE.CanvasTexture(canvas);tex.colorSpace=THREE.SRGBColorSpace;const mat=new THREE.MeshStandardMaterial({map:tex,roughness:.85,emissive:0xffffff,emissiveMap:tex,emissiveIntensity:glow?.6:.12,side:THREE.DoubleSide});const mesh=new THREE.Mesh(new THREE.PlaneGeometry(width,height),mat);mesh.position.set(...p);mesh.rotation.y=angle;group.add(mesh);return mesh;}
  ...
  function lantern(x,z){const m=new THREE.Mesh(new THREE.SphereGeometry(.25,10,8),material(0xf6ad60));m.scale.set(.8,1.4,.8);m.position.set(x,2.55,z);group.add(m);lamps.push(m.material);box([.26,.05,.26],[x,2.91,z],0x3c3430);}
  ...
  sites.forEach((s,i)=>{
    ...
    // Canvas awning, hanging noren and doorway with a recessed threshold.
    ...
    // Outdoor compressor, drainpipe and a little potted tree.
    ...
    shape('sphere',[.4,7,5],[side*6.75,.78,z-4.25],0x627857);
    obstacle(side*6.75,z-4.25,.65,.65);
    // Back streets and residential silhouettes.
    box([8,7+i%4,12],[side*22,(7+i%4)/2,z+4],0x848d87,[0,0,0],'wall');
    for(let n=0;n<3;n++)box([.08,1.5,1.4],[side*17.95,5.5,z+n*3],0x486168);
  });
  // Wooden utility poles, ceramic insulators and sagging cables.
  for(const side of [-1,1])for(let z=-48;z<=48;z+=16){
    cyl(.13,8,[side*6.7,4,z],0x5f554b);obstacle(side*6.7,z,.38,.38);
    box([2.4,.14,.18],[side*6.7,7.3,z],0x51584f);
    ...
    if(!mobile&&z%32===16){const light=new THREE.PointLight(0xffc17b,.7,10,2);light.position.set(side*5.5,4.8,z);group.add(light);}
  }
  // Pedestrian shopping-street gateway.
  ...
  // Vending machine, payphone, bus stop, arcade and ramen counter.
  ...
  cyl(.05,2.8,[-5.9,1.4,44],0x64756d);label('バス停','HARBOUR LINE',[-5.9,2.6,44],1.1,.75);anchor([-5.8,1,44],'Read bus timetable',()=>onAction('bus'));
  ...
  // Bicycles are simple original game meshes, with wheels and frames.
  for(const [x,z] of [[-6,6],[6,-6],[-6,-35]]){for(const dz of [-.62,.62]){const tire=new THREE.Mesh(new THREE.TorusGeometry(.36,.035,5,16),material(0x333b3d));tire.rotation.y=Math.PI/2;tire.position.set(x,.4,z+dz);group.add(tire);}beam([x,.4,z-.62],[x,.9,z],.035,0x71999a);beam([x,.9,z],[x,.4,z+.62],.035,0x71999a);beam([x,.4,z-.62],[x,.4,z+.35],.035,0x71999a);beam([x,.4,z+.62],[x,1.14,z+.55],.03);beam([x-.24,1.14,z+.55],[x+.24,1.14,z+.55],.03);box([.24,.07,.32],[x,1,z-.1],0x463d32);}
  // Harbour promenade, water, breakwater, boat and fishing point.
  box([38,.35,12],[0,-.08,-58],0xa5a699,[0,0,0],'wall');
  const sea=new THREE.Mesh(new THREE.PlaneGeometry(160,85,30,20),new THREE.MeshStandardMaterial({color:0x456f78,roughness:.3,metalness:.32}));sea.rotation.x=-Math.PI/2;sea.position.set(0,-.48,-106);group.add(sea);water.push(sea);
  ...
  for(const x of [-12,-8,8,12]){box([1.8,.14,.6],[x,.6,-59],0x927c56,[0,0,0],'wood');for(const dx of [-.65,.65])box([.12,.6,.4],[x+dx,.3,-59],0x4a5858);}
  const boat=new THREE.Group();boat.position.set(9,0,-70);group.add(boat);const hull=new THREE.Mesh(new THREE.BoxGeometry(3.2,.9,6.5),material(0x334e5c));boat.add(hull);const cabin=new THREE.Mesh(new THREE.BoxGeometry(2.3,1.7,2.3),material(0xd9d2bb));cabin.position.set(0,1,0);boat.add(cabin);const glass=new THREE.Mesh(new THREE.BoxGeometry(2.34,.65,2.35),material(0x426b77));glass.position.y=1.5;boat.add(glass);
  box([65,2,4],[0,.2,-97],0x7a8987);for(const x of [-26,26]){cyl(1,7,[x,3.5,-97],0xcbc9b6);box([2.3,.7,2.3],[x,7,-97],0x984e42);}
  // Small neighbourhood shrine at the south end.
  for(const x of [-3,3])cyl(.16,4,[x,2,55],0xa34531);box([7.3,.24,.36],[0,4.1,55],0x973f30);box([6.7,.18,.32],[0,3.45,55],0x973f30);box([3,2.6,2],[0,1.3,60],0x8b7050,[0,0,0],'wood');box([3.8,.22,2.8],[0,2.8,60],0x4e6061);anchor([0,1,57.8],'Visit the shrine',()=>onAction('shrine'));
  // Scheduled residents with articulated limbs.
  ...
  const cat=new THREE.Group();cat.position.set(-5,0,-25);group.add(cat);const cb=new THREE.Mesh(new THREE.BoxGeometry(.3,.3,.65),material(0xd0a471));cb.position.y=.28;cat.add(cb);const ch=new THREE.Mesh(new THREE.SphereGeometry(.2,8,6),material(0xd0a471));ch.position.set(0,.49,-.3);cat.add(ch);for(const x of [-.11,.11]){const ear=new THREE.Mesh(new THREE.ConeGeometry(.085,.18,3),material(0xd0a471));ear.position.set(x,.67,-.3);cat.add(ear);}register(cat,'Greet the cat',()=>onAction('cat'));
  for(const {geo,mat,matrices} of batches.values()){const m=new THREE.InstancedMesh(geo,mat,matrices.length);matrices.forEach((matrix,i)=>m.setMatrixAt(i,matrix));m.castShadow=!mobile;m.receiveShadow=true;group.add(m);}
  let wet=false;const rainCount=mobile?280:650,positions=new Float32Array(rainCount*3);for(let i=0;i<rainCount;i++){positions[i*3]=(Math.random()-.5)*32;positions[i*3+1]=Math.random()*16;positions[i*3+2]=(Math.random()-.5)*120;}const rainGeo=new THREE.BufferGeometry();rainGeo.setAttribute('position',new THREE.BufferAttribute(positions,3));const rain=new THREE.Points(rainGeo,new THREE.PointsMaterial({color:0xc7e0df,size:.055,transparent:true,opacity:.65}));rain.visible=false;group.add(rain);
  return {group,colliders,people,setRain(value){wet=value;rain.visible=value;material(0xcccccc,'road').roughness=value?.28:.87;},update(dt,time,day){boat.rotation.z=Math.sin(time*.7)*.025;boat.position.y=Math.sin(time*.9)*.07;sea.position.y=-.48+Math.sin(time*.4)*.025;for(const m of lamps){m.emissive.set(0xf6ad60);m.emissiveIntensity=1-day*.75;}people.forEach(p=>{p.g.position.z=p.z+Math.sin(time*.11+p.index)*2.2;p.g.rotation.y=Math.cos(time*.11+p.index)>0?Math.PI:0;p.legs.forEach((l,i)=>l.rotation.x=Math.sin(time*3+i*Math.PI)*.22);p.arms.forEach((l,i)=>l.rotation.x=-Math.sin(time*3+i*Math.PI)*.16);});cat.rotation.y=Math.sin(time*.3)*.2;if(wet){for(let i=0;i<rainCount;i++){positions[i*3+1]-=dt*12;if(positions[i*3+1]<0)positions[i*3+1]=16;}rainGeo.attributes.position.needsUpdate=true;}}};
}
