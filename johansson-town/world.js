...
export function createTown({scene,sites,mobile,register,enter,onAction,getPlayerPosition}) {
  ...
  sites.forEach((s,i)=>{
    ...
    for(const dz of [-3,-.2,2.6]){box([.07,1.45,1.7],[front-side*.1,5.12,z+dz],0x425c60);for(const offset of [-.88,0,.88])box([.18,1.7,.075],[front-side*.19,5.12,z+dz+offset],0x675d4b);for(const y of [4.33,5.9])box([.18,.08,1.85],[front-side*.19,y,z+dz],0x675d4b);box([1,.09,2],[front-side*.38,4.24,z+dz],0x68685c);}
  ...
  for(const side of [-1,1])for(let z=-48;z<=48;z+=16){cyl(.13,8,[side*6.7,4,z],0x5f554b);obstacle(side*6.7,z,.38,.38);box([2.4,.14,.18],[side*6.7,7.3,z],0x51584f);for(const dx of [-.8,0,.8]){cyl(.08,.26,[side*6.7+dx,7.52,z],0xd5d7cc);if(z<48){const points=[];for(let k=0;k<=8;k++)points.push(new THREE.Vector3(side*6.7+dx,7.58-Math.sin(k/8*Math.PI)*.75,z+k*2));const g=new THREE.BufferGeometry().setFromPoints(points);group.add(new THREE.Line(g,new THREE.LineBasicMaterial({color:0x29373b})));}}beam([side*6.7,5.7,z],[side*5.6,5.7,z],.06);box([.6,.12,.26],[side*5.5,5.65,z],0xe5c590);if(!mobile&&z%32===16){const light=new THREE.PointLight(0xffc17b,.7,10,2);light.position.set(side*5.5,4.8,z);group.add(light);}}
